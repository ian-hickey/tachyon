...
class ComponentAsExpression(tc: TagComponent?) : ExpressionBase(tc.getFactory(), tc.getStart(), tc.getEnd()) {
    ...
    override fun _writeOut(bc: BytecodeContext?, mode: Int): Type? {
        ...
        adapter.visitMethodInsn(Opcodes.INVOKEVIRTUAL, pageClassName, "getPageSource", "()Ltachyon/runtime/PageSource;")
        adapter.visitMethodInsn(Opcodes.INVOKESPECIAL, inlineClassName, "<init>", "(Ltachyon/runtime/PageSource;)V")
