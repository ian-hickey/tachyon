...
object TimeZoneUtil {
    ...
    init {
        ...
        for (i in lucee.commons.date.ids.indices) {
            IDS.put(lucee.commons.date.ids.get(i).toLowerCase(), TimeZone.getTimeZone(lucee.commons.date.ids.get(i)))
        }
        val def: TimeZone = TimeZone.getDefault()
        if (lucee.commons.date.def != null) IDS.put(lucee.commons.date.def.getID(), lucee.commons.date.def)
