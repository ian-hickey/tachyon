...
class Page(factory: Factory?, config: Config?, sc: SourceCode?, tc: TagCIObject?, version: Long, lastModifed: Long, writeLog: Boolean, suppressWSbeforeArg: Boolean, output: Boolean,
    ...
    override fun execute(className: String?): ByteArray? {
        ...
        // parent
        var parent: String? = PageImpl::class.java.getName() // "tachyon/runtime/Page";
        var interfaces: Array<String?>? = null
        if (isComponent(comp)) {
            parent = ComponentPageImpl::class.java.getName() // "tachyon/runtime/ComponentPage";
            if (isSub) interfaces = arrayOf(SubPage::class.java.getName().replace('.', '/'))
        } else if (isInterface(comp)) parent = InterfacePageImpl::class.java.getName() // "tachyon/runtime/InterfacePage";
        ...
        run {
            val fv: FieldVisitor = cw.visitField(Opcodes.ACC_PRIVATE + Opcodes.ACC_FINAL, "imports", "[Ltachyon/runtime/component/ImportDefintion;", null, null)
            fv.visitEnd()
            constrAdapter.visitVarInsn(Opcodes.ALOAD, 0)
            val av = ArrayVisitor()
            ...
            av.visitEnd()
            constrAdapter.visitFieldInsn(Opcodes.PUTFIELD, className, "imports", "[Ltachyon/runtime/component/ImportDefintion;")
        }
        ...
        if (imports.size() > 0) {
            ...
            adapter.visitFieldInsn(Opcodes.GETFIELD, className, "imports", "[Ltachyon/runtime/component/ImportDefintion;")
            adapter.returnValue()
            adapter.endMethod()
        } else {
            adapter = GeneratorAdapter(Opcodes.ACC_PUBLIC + Opcodes.ACC_FINAL, GET_IMPORT_DEFINITIONS, null, null, cw)
            adapter.visitInsn(Opcodes.ICONST_0)
            adapter.visitTypeInsn(Opcodes.ANEWARRAY, "tachyon/runtime/component/ImportDefintion")
            adapter.returnValue()
            adapter.endMethod()
        }
        ...
        if (isInterface()) {
            ...
            run {
                ...
                while (i < functions.size) {
                    ...
                    adapter.visitMethodInsn(Opcodes.INVOKEVIRTUAL, className, createFunctionName(++count), "(Ltachyon/runtime/PageContext;Ltachyon/runtime/type/UDF;I)Ljava/lang/Object;")
        ...
        if (isInterface()) {
            ...
            run {
                ...
                while (i < functions.size) {
                    ...
                    adapter.visitMethodInsn(Opcodes.INVOKEVIRTUAL, className, "udfDefaultValue" + ++count, "(Ltachyon/runtime/PageContext;IILjava/lang/Object;)Ljava/lang/Object;")
                    adapter.visitInsn(Opcodes.ARETURN) // adapter.returnValue();
                    cv.visitWhenAfterBody(bc)
                    i += 10
                }
        ...
        // set field subs
        val fv: FieldVisitor = cw.visitField(Opcodes.ACC_PRIVATE + Opcodes.ACC_FINAL, "subs", "[Ltachyon/runtime/CIPage;", null, null)
        fv.visitEnd()
    ...
    private fun writeGetSubPages(cw: ClassWriter?, name: String?, subs: List<TagCIObject?>?, dialect: Int) {
        ...
        val endIF = Label()
        adapter.visitVarInsn(Opcodes.ALOAD, 0)
        adapter.visitFieldInsn(Opcodes.GETFIELD, name, "subs", "[Ltachyon/runtime/CIPage;")
        ...
        while (it.hasNext()) {
            ...
            adapter.visitVarInsn(Opcodes.ALOAD, 0)
            adapter.visitMethodInsn(Opcodes.INVOKEVIRTUAL, name, "getPageSource", "()Ltachyon/runtime/PageSource;")
            adapter.visitMethodInsn(Opcodes.INVOKESPECIAL, className, "<init>", "(Ltachyon/runtime/PageSource;)V")
            av.visitEndItem(adapter)
        }
        av.visitEnd()
        adapter.visitFieldInsn(Opcodes.PUTFIELD, name, "subs", "[Ltachyon/runtime/CIPage;")
    ...
    private fun writeOutGetStaticStruct(constr: ConstrBytecodeContext?, keys: List<LitString?>?, cw: ClassWriter?, component: TagCIObject?, name: String?) {
        ...
        val fv: FieldVisitor = cw.visitField(Opcodes.ACC_PRIVATE + Opcodes.ACC_STATIC + Opcodes.ACC_FINAL, "staticStruct", "Ltachyon/runtime/component/StaticStruct;", null, null)
        fv.visitEnd()
        run {
    ...
    private fun writeOutNewComponent(constr: ConstrBytecodeContext?, keys: List<LitString?>?, cw: ClassWriter?, component: Tag?, name: String?) {
        ...
        if (attr != null) {
            ...
            modifiers = ComponentUtil.toModifier(ls.getString(), tachyon.runtime.Component.MODIFIER_NONE, tachyon.runtime.Component.MODIFIER_NONE)
        }
    ...
    fun isComponent(): Boolean {
		 ...
		 * comp.getTagLibTag().getTagClassName().equals("tachyon.runtime.tag.Component")) return true; return
		 * false;
		 */
    }
    ...
    fun isInterface(): Boolean {
        ...
        /*
		 * TagCFObject comp = getTagCFObject(null); if(comp!=null &&
		 * comp.getTagLibTag().getTagClassName().equals("tachyon.runtime.tag.Interface")) return true; return
		 * false;
		 */
    }
    ...
    companion object {
        val NULL: Type? = Type.getType(tachyon.runtime.type.Null::class.java)
	 ...
	 */
        ...
        // void call (tachyon.runtime.PageContext)
