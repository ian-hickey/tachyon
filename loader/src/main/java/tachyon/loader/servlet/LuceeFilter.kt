...
class TachyonFilter : Filter {
    ...
    fun doFilter(request: ServletRequest, response: ServletResponse?, chain: FilterChain?) {
        try {
            ...
            engine.addServletConfig(TachyonFilterImpl(request, response, chain, "filter"))
        } catch (se: Exception) {
            se.printStackTrace()
        }
    }
    ...
    class TachyonFilterImpl(request: ServletRequest, response: ServletResponse?, chain: FilterChain?, status: String) : ServletConfig {
