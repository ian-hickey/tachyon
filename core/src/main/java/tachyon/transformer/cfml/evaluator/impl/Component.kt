    ...
    fun evaluate(tag: Tag?, tlt: TagLibTag?) {
        ...
        if (!inline && isInsideCITemplate(page) === Boolean.FALSE) {
            throw EvaluatorException("Wrong Context, [" + tlt.getFullName().toString() + "] tag must be inside a file with the extension [" + Constants.getCFMLComponentExtension()
                    .toString() + "] or [" + Constants.getTachyonComponentExtension().toString() + "], only inline components are allowed outside ")
        }
        val isComponent: Boolean = tlt.getTagClassDefinition().isClassNameEqualTo("tachyon.runtime.tag.Component")
        /*
		 * boolean isInterface="tachyon.runtime.tag.Interface".equals(tlt.getTagClassName()); if(main) {
        ...
        // output
        // "output=true" is handled in "tachyon.transformer.cfml.attributes.impl.Function"
        ...
        if (isComponent) {
            ...
            if (attr != null) {
                ...
                val mod: Int = ComponentUtil.toModifier(ls.getString(), tachyon.runtime.Component.MODIFIER_NONE, -1)
