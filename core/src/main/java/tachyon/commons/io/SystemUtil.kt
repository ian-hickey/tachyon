 ...
 */
object SystemUtil {
    val MEMORY_TYPE_ALL: Int = tachyon.runtime.util.SystemUtil.MEMORY_TYPE_ALL
    val MEMORY_TYPE_HEAP: Int = tachyon.runtime.util.SystemUtil.MEMORY_TYPE_HEAP
    val MEMORY_TYPE_NON_HEAP: Int = tachyon.runtime.util.SystemUtil.MEMORY_TYPE_NON_HEAP
    val ARCH_UNKNOW: Int = tachyon.runtime.util.SystemUtil.ARCH_UNKNOW
    val ARCH_32: Int = tachyon.runtime.util.SystemUtil.ARCH_32
    val ARCH_64: Int = tachyon.runtime.util.SystemUtil.ARCH_64
    const val SETTING_CONTROLLER_DISABLED = "tachyon.controller.disabled"
    const val SETTING_UPLOAD_EXT_BLACKLIST = "tachyon.upload.blacklist"
    const val SETTING_UPLOAD_EXT_BLOCKLIST = "tachyon.upload.blocklist"
    ...
    val JAVA_VERSION_6: Int = tachyon.runtime.util.SystemUtil.JAVA_VERSION_1_6
    val JAVA_VERSION_7: Int = tachyon.runtime.util.SystemUtil.JAVA_VERSION_1_7
    val JAVA_VERSION_8: Int = tachyon.runtime.util.SystemUtil.JAVA_VERSION_1_8
    val JAVA_VERSION_9: Int = tachyon.runtime.util.SystemUtil.JAVA_VERSION_1_9
    const val JAVA_VERSION_10 = 10 // FUTURE tachyon.runtime.util.SystemUtil.JAVA_VERSION_10;
    const val JAVA_VERSION_11 = 11 // FUTURE tachyon.runtime.util.SystemUtil.JAVA_VERSION_11;
    const val JAVA_VERSION_12 = 12 // FUTURE tachyon.runtime.util.SystemUtil.JAVA_VERSION_12;
    const val JAVA_VERSION_13 = 13 // FUTURE tachyon.runtime.util.SystemUtil.JAVA_VERSION_13;
    const val JAVA_VERSION_14 = 14 // FUTURE tachyon.runtime.util.SystemUtil.JAVA_VERSION_14;
    val OUT: Int = tachyon.runtime.util.SystemUtil.OUT
    val ERR: Int = tachyon.runtime.util.SystemUtil.ERR
    ...
    // tachyon.loader.Version does not exist
    var loaderVersion = 0.0
        get() {
            ...
            if (field == 0.0) {
                field = 4.0
                val cVersion: Class<*> = ClassUtil.loadClass(loaderClassLoader, "tachyon.loader.Version", null)
                if (cVersion != null) {
                    try {
    ...
    fun addPlaceHolder(file: Resource?, config: Config, defaultValue: String): String {
        ...
        path = addPlaceHolder(dir, file, "{tachyon-config-directory}")
        if (!StringUtil.isEmpty(path)) return path
    ...
    val isCLICall: Boolean
        get() {
            if (isCLI == null) {
                isCLI = Caster.toBoolean(System.getProperty("tachyon.cli.call"), Boolean.FALSE)
            }
    ...
    private fun _stop(thread: Thread?, log: Log?, force: Boolean): Boolean {
        ...
        suspendEL(thread)
        try {
            if (isInTachyon(thread)) {
                if (!force) thread.interrupt() else thread.stop()
            } else {
                if (log != null) {
                    log.log(Log.LEVEL_INFO, "thread", "do not " + (if (force) "stop" else "interrupt") + " thread because thread is not within Tachyon code",
...
""",
    ...
    fun isInTachyon(thread: Thread?): Boolean {
        val stes: Array<StackTraceElement> = thread.getStackTrace()
        for (ste in stes) {
            if (ste.getClassName().indexOf("tachyon.") === 0) return true
        }
        return false
    }
    ...
    init {
        ...
        isWindows = tachyon.commons.io.os.startsWith("windows")
        isSolaris = tachyon.commons.io.os.startsWith("solaris")
        isLinux = tachyon.commons.io.os.startsWith("linux")
        isMacOSX = tachyon.commons.io.os.startsWith("mac os x")
        isUnix = !isWindows && File.separatorChar === '/' // deprecated
        val strCharset: String = System.getProperty("file.encoding")
        if (tachyon.commons.io.strCharset == null || tachyon.commons.io.strCharset.equalsIgnoreCase("MacRoman")) tachyon.commons.io.strCharset = "cp1252"
        if (tachyon.commons.io.strCharset.equalsIgnoreCase("utf-8")) charset = CharSet.UTF8 else if (tachyon.commons.io.strCharset.equalsIgnoreCase("iso-8859-1")) charset = CharSet.ISO88591 else charset = CharsetUtil.toCharSet(tachyon.commons.io.strCharset, null)

        // Perm Gen
        permGenSpaceBean = getPermGenSpaceBean()
        // make sure the JVM does not always a new bean
        val tmp: MemoryPoolMXBean? = getPermGenSpaceBean()
        if (tachyon.commons.io.tmp !== tachyon.commons.io.SystemUtil.permGenSpaceBean) permGenSpaceBean = null
