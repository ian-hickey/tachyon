component extends = "org.tachyon.cfml.test.TachyonTestCase" labels="application" skip="true" {
	...
	function run( testResults , testBox ) {
		describe("testcase for LDEV-3613", function( currentSpec ){
			it(title="Checking applicationStop() with multiple requests", body=function( currentSpec ){
				...
				var results="";
				loop array=listToArray(threadNames) item="local.name" {
					var threads=cfthread[name];
