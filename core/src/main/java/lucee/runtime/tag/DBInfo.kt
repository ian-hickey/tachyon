...
class DBInfo : TagImpl() {
    ...
    private fun typeDBNames(conn: Connection?) {
        ...
        stopwatch.start()
        val metaData: DatabaseMetaData = conn.getMetaData()
        val catalogs: lucee.runtime.type.Query = QueryImpl(metaData.getCatalogs(), "query", pageContext.getTimeZone())
        val scheme: lucee.runtime.type.Query = QueryImpl(metaData.getSchemas(dbname(conn), null), "query", pageContext.getTimeZone())
        var p: Pattern? = null
        ...
        val qry: lucee.runtime.type.Query = QueryImpl(columns, types, 0, "query")
        var row = 1
    ...
    private fun typeForeignKeys(conn: Connection?) {
        ...
        val qry: lucee.runtime.type.Query = QueryImpl(metaData.getExportedKeys(_dbName, schema, table), "query", pageContext.getTimeZone())
        qry.setExecutionTime(stopwatch!!.time())
        pageContext.setVariable(name, qry)
    }
    ...
    private fun typeIndex(conn: Connection?) {
        ...
        val qry: lucee.runtime.type.Query = QueryImpl(tables, "query", pageContext.getTimeZone())
    ...
    private fun typeProcedures(conn: Connection?) {
        ...
        val qry: lucee.runtime.type.Query = QueryImpl(metaData.getProcedures(dbname(conn), schema, if (StringUtil.isEmpty(pattern)) "%" else pattern), "query", pageContext.getTimeZone())
        qry.setExecutionTime(stopwatch!!.time())
        pageContext.setVariable(name, qry)
    }
    ...
    private fun typeProcedureColumns(conn: Connection?) {
        ...
        val qry: lucee.runtime.type.Query = QueryImpl(
    ...
    private fun typeTables(conn: Connection?) {
        ...
        stopwatch.start()
        pattern = setCase(metaData, pattern)
        val qry: lucee.runtime.type.Query = QueryImpl(metaData.getTables(dbname(conn), null, if (StringUtil.isEmpty(pattern)) "%" else pattern,
    ...
    private fun typeVersion(metaData: DatabaseMetaData?) {
        ...
        val qry: lucee.runtime.type.Query = QueryImpl(columns, types, 1, "query")
