...
class CLIInvokerImpl(root: File?, servletName: String?) : CLIInvoker {
    ...
    init {
        ...
        val param: String = Util._getSystemPropOrEnvVar("lucee.cli.config", null)
        if (param != null && !param.isEmpty()) {
            initParams.put("lucee-web-directory", File(param, "lucee-web").getAbsolutePath())
            initParams.put("lucee-server-directory", File(param).getAbsolutePath()) // will create a subfolder named lucee-server
        } else initParams.put("lucee-server-directory", File(root, "WEB-INF").getAbsolutePath())
