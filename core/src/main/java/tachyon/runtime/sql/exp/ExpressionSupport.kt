...
abstract class ExpressionSupport : Expression {
    @get:Override
    @set:Override
    override var index = 0
    private override var alias: String? = null
    ...
    @get:Override
    @set:Override
    override var isDirectionBackward = false
    @Override
    fun getAlias(): String? {
        return if (alias == null) "column_" + (index - 1) else alias
    }
    ...
    @Override
    override fun hasAlias(): Boolean {
        return alias != null
    }

    @Override
    override fun hasIndex(): Boolean {
        return index != 0
    }
