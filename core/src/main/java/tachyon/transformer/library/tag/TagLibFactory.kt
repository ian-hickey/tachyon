...
class TagLibFactory : DefaultHandler {
    ...
    companion object {
        ...
        private val TLD_LUCEE: String? = "/resource/tld/core-tachyon.tld"
        ...
        private fun loadFromSystem(id: Identification?): Array<TagLib?>? {
            if (systemTLDs!![CFMLEngine.DIALECT_CFML] == null) {
                ...
                val tachyon: TagLib = cfml!!.duplicate(false)
                tachyon.transformer.library.tag.TagLibFactory.Companion.systemTLDs.get(CFMLEngine.DIALECT_CFML) = tachyon.transformer.library.tag.TagLibFactory(cfml, tachyon.transformer.library.tag.TagLibFactory.Companion.TLD_CFML, id).getLib()
                tachyon.transformer.library.tag.TagLibFactory.Companion.systemTLDs.get(CFMLEngine.DIALECT_LUCEE) = tachyon.transformer.library.tag.TagLibFactory(tachyon, tachyon.transformer.library.tag.TagLibFactory.Companion.TLD_LUCEE, id).getLib()
            }
            return systemTLDs
        }
