...
object GetFileInfo {
    ...
    fun call(pc: PageContext?, oSrc: Object?): Struct? {
        val src: Resource = Caster.toResource(pc, oSrc, true)
        val file = File(Caster.toString(oSrc))
        ...
        sct.set("lastmodified", DateTimeImpl(pc, src.lastModified(), false))
        try {
        ...
        if (src.isDirectory()) sct.set(KeyConstants._type, "directory") else if (src.isFile()) sct.set(KeyConstants._type, "file") else sct.set(KeyConstants._type, "")

        // supported only by tachyon
