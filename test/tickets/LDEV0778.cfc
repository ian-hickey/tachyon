...
,"A"';
	...
	public void function testWhiteCommaAtTheEnd(){
		var qry=CSVParser.toQuery(str2, ',', '"', nullValue(), true );
		assertEquals(3,qry.recordcount());
		assertEquals(2,qry.columnCount());
	}

	public void function testWhiteCommaAtTheEnd2(){
		var qry=CSVParser.toQuery(str3, ',', '"', nullValue(), true );
		assertEquals(3,qry.recordcount());
		assertEquals(2,qry.columnCount());
	}
