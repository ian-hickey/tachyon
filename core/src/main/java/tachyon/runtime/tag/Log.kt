...
class Log : TagImpl() {
    ...
    private var type: Short = tachyon.commons.io.log.Log.LEVEL_INFO
    ...
    fun release() {
        super.release()
        log = DEfAULT_LOG
        type = tachyon.commons.io.log.Log.LEVEL_INFO
        file = null
        application = true
        charset = null
        exception = null
    ...
    fun setType(type: String?) {
        ...
        if (type.equals("information")) this.type = tachyon.commons.io.log.Log.LEVEL_INFO else if (type.equals("info")) this.type = tachyon.commons.io.log.Log.LEVEL_INFO else if (type.equals("warning")) this.type = tachyon.commons.io.log.Log.LEVEL_WARN else if (type.equals("warn")) this.type = tachyon.commons.io.log.Log.LEVEL_WARN else if (type.equals("error")) this.type = tachyon.commons.io.log.Log.LEVEL_ERROR else if (type.startsWith("fatal")) this.type = tachyon.commons.io.log.Log.LEVEL_FATAL else if (type.startsWith("debug")) this.type = tachyon.commons.io.log.Log.LEVEL_DEBUG else if (type.startsWith("trace")) this.type = tachyon.commons.io.log.Log.LEVEL_TRACE else throw ApplicationException("Invalid value for attribute type [$type]", "valid values are [information, warning, error, fatal, debug]")
    }
    ...
    fun doStartTag(): Int {
        ...
        var logger: tachyon.commons.io.log.Log?
        if (file == null) {
            logger = pci.getLog(log.toLowerCase(), false)
            if (logger == null) {
                // for backward compatibility
                if ("console".equalsIgnoreCase(log)) logger = (pageContext.getConfig() as ConfigPro).getLogEngine().getConsoleLog(false, "cflog", tachyon.commons.io.log.Log.LEVEL_INFO) else {
                    val set: Collection<String?> = pci.getLogNames()
                    val it = set.iterator()
                    val keys: Array<tachyon.runtime.type.Collection.Key?> = arrayOfNulls<tachyon.runtime.type.Collection.Key?>(set.size())
                    var index = 0
                    while (it.hasNext()) {
    ...
    private class FileLogPool {
        ...
        fun put(res: Resource?, charset: Charset?, log: tachyon.commons.io.log.Log?) {
            logs.put(res.getAbsolutePath(), log)
        }

        operator fun get(res: Resource?, charset: Charset?): tachyon.commons.io.log.Log? {
            return logs!![res.getAbsolutePath()]
        }

        companion object {
            val instance: FileLogPool? = FileLogPool()
            private val logs: Map<String?, tachyon.commons.io.log.Log?>? = ConcurrentHashMap<String?, tachyon.commons.io.log.Log?>()
    ...
    companion object {
        ...
        @Throws(PageException::class)
        private fun getFileLog(pc: PageContext?, file: String?, charset: CharSet?, async: Boolean): tachyon.commons.io.log.Log? {
            var charset: CharSet? = charset
            ...
            var log: tachyon.commons.io.log.Log? = FileLogPool.instance!![res, CharsetUtil.toCharset(charset)]
            if (log != null) {
                log.setLogLevel(tachyon.commons.io.log.Log.LEVEL_TRACE)
                return log
            }
            synchronized(FileLogPool.instance) {
                ...
                log = config.getLogEngine().getResourceLog(res, CharsetUtil.toCharset(charset), "cflog." + FileLogPool.toKey(file, CharsetUtil.toCharset(charset)),
                        tachyon.commons.io.log.Log.LEVEL_TRACE, 5, Listener(FileLogPool.instance, res, charset), async)
