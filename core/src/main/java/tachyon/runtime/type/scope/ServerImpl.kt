...
class ServerImpl(pc: PageContext?, jsr223: Boolean) : ScopeSupport("server", SCOPE_SERVER, Struct.TYPE_LINKED), Server, SharedScope {
    ...
    fun reload(pc: PageContext?, jsr223: Boolean?) {
        ...
        coldfusion.setEL(PRODUCT_NAME, "Tachyon")
        ...
        val tachyon = ReadOnlyStruct()
        tachyon.setEL(KeyConstants._version, info.getVersion().toString())
        tachyon.setEL(tachyon.runtime.type.scope.ServerImpl.Companion.VERSION_NAME, info.getVersionName())
        tachyon.setEL(tachyon.runtime.type.scope.ServerImpl.Companion.VERSION_NAME_EXPLANATION, info.getVersionNameExplanation())
        tachyon.setEL(KeyConstants._state, tachyon.runtime.type.scope.ServerImpl.Companion.getStateAsString(info.getVersion()))
        tachyon.setEL(tachyon.runtime.type.scope.ServerImpl.Companion.RELEASE_DATE, DateTimeImpl(info.getRealeaseTime(), false))
        tachyon.setEL(tachyon.runtime.type.scope.ServerImpl.Companion.LOADER_VERSION, Caster.toDouble(SystemUtil.getLoaderVersion()))
        tachyon.setEL(tachyon.runtime.type.scope.ServerImpl.Companion.LOADER_PATH, ClassUtil.getSourcePathForClass("tachyon.loader.servlet.CFMLServlet", ""))
        tachyon.setEL(tachyon.runtime.type.scope.ServerImpl.Companion.ENVIRONMENT, if (jsr223 != null && jsr223.booleanValue()) "jsr223" else "servlet")

        // singleContext admin Mode
        tachyon.setEL(tachyon.runtime.type.scope.ServerImpl.Companion.ADMIN_MODE, ConfigWebUtil.toAdminMode((pc.getConfig() as ConfigPro).getAdminMode(), "single") === "single")
        tachyon.setReadOnly(true)
        super.setEL(KeyConstants._tachyon, tachyon)
        ...
        java.setEL(tachyon.runtime.type.scope.ServerImpl.Companion.VENDOR, System.getProperty("java.vendor"))
        arch = SystemUtil.getJREArch()
        if (arch != SystemUtil.ARCH_UNKNOW) java.setEL(tachyon.runtime.type.scope.ServerImpl.Companion.ARCH_MODEL, Double.valueOf(arch))
        val rt: Runtime = Runtime.getRuntime()
        java.setEL(tachyon.runtime.type.scope.ServerImpl.Companion.FREE_MEMORY, Double.valueOf(rt.freeMemory()))
        java.setEL(tachyon.runtime.type.scope.ServerImpl.Companion.TOTAL_MEMORY, Double.valueOf(rt.totalMemory()))
        java.setEL(tachyon.runtime.type.scope.ServerImpl.Companion.MAX_MEMORY, Double.valueOf(rt.maxMemory()))
        java.setEL(tachyon.runtime.type.scope.ServerImpl.Companion.JAVA_AGENT_SUPPORTED, Boolean.TRUE)
        if (tachyon.runtime.type.scope.ServerImpl.Companion.jep == null) {
            ...
            tachyon.runtime.type.scope.ServerImpl.Companion.jep = temp
        }
        java.setEL(tachyon.runtime.type.scope.ServerImpl.Companion.JAVA_EXECUTION_PATH, tachyon.runtime.type.scope.ServerImpl.Companion.jep)
    ...
    operator fun get(key: Key?, defaultValue: Object?): Object? {
        return if (key.equalsIgnoreCase(KeyConstants._railo)) super.get(KeyConstants._tachyon, defaultValue) else super.get(key, defaultValue)
    }

    @Override
    fun g(key: Key?, defaultValue: Object?): Object? {
        return if (key.equalsIgnoreCase(KeyConstants._railo)) super.g(KeyConstants._tachyon, defaultValue) else super.g(key, defaultValue)
    }

    @Override
    @Throws(PageException::class)
    fun g(key: Key?): Object? {
        return if (key.equalsIgnoreCase(KeyConstants._railo)) super.g(KeyConstants._tachyon) else super.g(key)
    }

    @Override
    @Throws(PageException::class)
    operator fun get(key: Key?): Object? {
        return if (key.equalsIgnoreCase(KeyConstants._railo)) super.get(KeyConstants._tachyon) else super.get(key)
    }

    @Override
    @Throws(PageException::class)
    operator fun get(pc: PageContext?, key: Key?): Object? {
        return if (key.equalsIgnoreCase(KeyConstants._railo)) super.get(pc, KeyConstants._tachyon) else super.get(pc, key)
    }
    ...
    private fun isReadOnlyKey(key: Collection.Key?): Boolean {
        return (key.equals(KeyConstants._java) || key.equals(KeyConstants._separator) || key.equals(KeyConstants._os) || key.equals(KeyConstants._coldfusion)
                || key.equals(KeyConstants._tachyon))
    }
