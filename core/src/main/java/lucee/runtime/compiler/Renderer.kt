...
object Renderer {
    ...
    private fun loadClass(config: ConfigWebPro?, className: String?, cfml: String?, dialect: Int, ignoreScopes: Boolean): Class<out Page?>? {
        ...
        // compile
        val result: lucee.runtime.compiler.CFMLCompilerImpl.Result = compiler!!.compile(config, sc, config.getTLDs(dialect), config.getFLDs(dialect), null, className, true, ignoreScopes)
