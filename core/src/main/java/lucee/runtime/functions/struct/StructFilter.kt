...
class StructFilter : BIF() {
    ...
    operator fun invoke(pc: PageContext?, args: Array<Object?>?): Object? {
        ...
        throw FunctionException(pc, "StructFilter", 2, 4, args.size)
    }

    companion object {
        @Throws(PageException::class)
        fun call(pc: PageContext?, sct: Struct?, udf: UDF?): Struct? {
            return _call(pc, sct, udf, false, 20)
        }

        @Throws(PageException::class)
        fun call(pc: PageContext?, sct: Struct?, udf: UDF?, parallel: Boolean): Struct? {
            return _call(pc, sct, udf, parallel, 20)
        }

        @Throws(PageException::class)
        fun call(pc: PageContext?, sct: Struct?, udf: UDF?, parallel: Boolean, maxThreads: Double): Struct? {
            return _call(pc, sct, udf, parallel, maxThreads.toInt())
        }

        @Throws(PageException::class)
        fun _call(pc: PageContext?, sct: Struct?, filter: UDF?, parallel: Boolean, maxThreads: Int): Struct? {
            ...
            return Filter.call(pc, sct, filter, parallel, maxThreads, Filter.TYPE_STRUCT) as Struct
        }
    }
}