...
class NtpMessage {
     ...
     */
    var referenceIdentifier: ByteArray? = byteArrayOf(0, 0, 0, 0)
    ...
    constructor(array: ByteArray?) {
        ...
        leapIndicator = (array!![0] shr 6 and 0x3)
        version = (array[0] shr 3 and 0x7)
        mode = (array[0] and 0x7) as Byte
     ...
     */
    constructor(leapIndicator: Byte, version: Byte, mode: Byte, stratum: Short, pollInterval: Byte, precision: Byte, rootDelay: Double, rootDispersion: Double,
                referenceIdentifier: ByteArray?, referenceTimestamp: Double, originateTimestamp: Double, receiveTimestamp: Double, transmitTimestamp: Double) {
    ...
    fun toByteArray(): ByteArray? {
        ...
        val p = ByteArray(48)
        p[0] = (leapIndicator shl 6 or (version shl 3) or mode) as Byte
        p[1] = stratum.toByte()
        p[2] = pollInterval
        p[3] = precision
    ...
    override fun toString(): String {
        val precisionStr: String = DecimalFormat("0.#E0").format(Math.pow(2, precision))
...
Transmit timestamp:  ${timestampToString(transmitTimestamp)}"""
    ...
    companion object {
        ...
        fun unsignedByteToShort(b: Byte): Short {
            return if (b and 0x80 == 0x80) (128 + (b and 0x7f)) as Short else b.toShort()
        }

        /**
         * Will read 8 bytes of a message beginning at `pointer` and return it as a double,
        ...
        fun encodeTimestamp(array: ByteArray?, pointer: Int, timestamp: Double) {
            ...
            for (i in 0..7) {
                ...
                // Capture byte value
                array!![pointer + i] = (timestamp / base).toByte()
            ...
            array!![7] = ThreadLocalRandom.current().nextInt(256) as Byte
        }
        ...
        fun timestampToString(timestamp: Double): String? {
            ...
            // date/time
            val date: String = DateFormatPool.format("dd-MMM-yyyy HH:mm:ss", Date(ms))

            // fraction
            val fraction = timestamp - timestamp.toLong()
            val fractionSting: String = DecimalFormat(".000000").format(fraction)
        ...
        fun referenceIdentifierToString(ref: ByteArray?, stratum: Short, version: Byte): String? {
            ...
            if (stratum.toInt() == 0 || stratum.toInt() == 1) {
                ...
                return unsignedByteToShort(ref!![0]).toString() + "." + unsignedByteToShort(ref[1]) + "." + unsignedByteToShort(ref[2]) + "." + unsignedByteToShort(ref[3])
