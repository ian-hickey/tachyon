...
object PageContextUtil {
    ...
    fun getCookieDomain(pc: PageContext?): String? {
        ...
        if (!StringUtil.isEmpty(result)) {
            ...
            if (!tachyon.runtime.op.Decision.isNumber(listLast)) { // if it's numeric then must be IP address
                var numparts = 2
        ...
        return try {
            ...
            if (config is ConfigWeb) {
                ...
                if (servletConfig == null) {
                    ...
                    for (conf in configs) {
                        if (tachyon.commons.io.SystemUtil.arePathsSame(rootDir, ReqRspUtil.getRootPath(conf.getServletContext()))) {
                            servletConfig = conf
                            break
                        }
                    }
            ...
            factory.getTachyonPageContext(servlet, req, rsp, null, false, -1, false, register, timeout, false, ignoreScopes)
        } finally {
    ...
    fun releasePageContext(pc: PageContext?, register: Boolean) {
        if (pc != null) pc.getConfig().getFactory().releaseTachyonPageContext(pc, register)
