...
class TagTry2(f: Factory?, start: Position?, end: Position?) : TagBase(f, start, end), FlowControlRetry {
    ...
    override fun _writeOut(bc: BytecodeContext?) {
        ...
        run {
            ...
            while (it.hasNext()) {
                ...
                if (stat is Tag) {
                    tag = stat
                    if (tag!!.getTagLibTag().getTagClassDefinition().isClassNameEqualTo("tachyon.runtime.tag.Catch")) {
                        catches.add(tag)
                        continue
                    } else if (tag!!.getTagLibTag().getTagClassDefinition().isClassNameEqualTo("tachyon.runtime.tag.Finally")) {
                        tmpFinal = tag
                        continue
                    }
                }
        ...
        // if(e instanceof tachyon.runtime.exp.Abort) throw e;
        val abortEnd = Label()
        adapter.loadLocal(e)
        // Abort.isAbort(t);
    ...
    private fun hasFinally(): Boolean {
        ...
        while (it.hasNext()) {
            ...
            if (stat is Tag) {
                tag = stat
                if (tag!!.getTagLibTag().getTagClassDefinition().isClassNameEqualTo("tachyon.runtime.tag.Finally")) {
                    return true
                }
            }
        }
