...
class Query : BodyTagTryCatchFinallyImpl() {
    ...
    companion object {
        ...
        private fun createMetaData(pageContext: PageContext?, data: QueryBean?, queryResult: QueryResult?, sqlQuery: SQL?, setVars: Boolean, exe: Long): Struct? {
            ...
            if (data.result != null && queryResult != null) {
                ...
                if (queryResult.getColumncount() + queryResult.getRecordcount() > 0) {
                    val list: String = ListUtil.arrayToList(if (queryResult is lucee.runtime.type.Query) (queryResult as lucee.runtime.type.Query?).getColumnNamesAsString() else CollectionUtil.toString(queryResult.getColumnNames(), false), ",")
                ...
                // GENERATED KEYS
                val qi: lucee.runtime.type.Query = Caster.toQuery(queryResult, null)
                if (qi != null) {
                    val qryKeys: lucee.runtime.type.Query = qi.getGeneratedKeys()
                    if (qryKeys != null) {
        ...
        @Throws(PageException::class)
        private fun executeQoQ(pc: PageContext?, data: QueryBean?, sql: SQL?, tl: TemplateLine?): lucee.runtime.type.QueryImpl? {
            return try {
