...
class DollarFormat : BIF() {
    @Override
    @Throws(PageException::class)
    operator fun invoke(pc: PageContext?, args: Array<Object?>?): Object? {
        if (args!!.size == 1) return call(pc, Caster.toString(args[0]))
        throw FunctionException(pc, "DollarFormat", 1, 1, args.size)
    }

    companion object {
        private const val serialVersionUID = -9006485495830329674L
        @Throws(PageException::class)
        fun call(pc: PageContext?, strDollar: String?): String? {
            var strDollar = strDollar
