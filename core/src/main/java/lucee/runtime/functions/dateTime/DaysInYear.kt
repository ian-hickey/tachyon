...
class DaysInYear : BIF() {
    ...
    companion object {
        private const val serialVersionUID = -2900647153777735688L
        ...
        private fun _call(pc: PageContext?, date: DateTime?, tz: TimeZone?): Double {
            val util: DateTimeUtil = DateTimeUtil.getInstance()
            return if (util.isLeapYear(util.getYear(tz, date))) 366 else 365
        }
    }
}