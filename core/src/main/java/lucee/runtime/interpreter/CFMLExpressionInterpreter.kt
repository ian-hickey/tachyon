...
class CFMLExpressionInterpreter @JvmOverloads constructor(limited: Boolean = true) {
    ...
    private fun subDynamic(ref: Ref?): Ref? {
        ...
        if (ref is lucee.runtime.interpreter.ref.`var`.Scope) {
            val s: lucee.runtime.interpreter.ref.`var`.Scope? = ref
    ...
    private fun identifier(firstCanBeNumber: Boolean): String? {
        ...
        doUpper = if (ps != null) !isJson && ps.getDialect() === CFMLEngine.DIALECT_CFML && (ps.getMapping() as MappingImpl).getDotNotationUpperCase() else !isJson && (config as ConfigWebPro?).getDotNotationUpperCase() // MUST .lucee should not be upper case
