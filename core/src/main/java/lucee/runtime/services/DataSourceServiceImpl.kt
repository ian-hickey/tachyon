...
class DataSourceServiceImpl : ServiceSupport(), DataSourceService {
    ...
    fun getDrivers(): Struct? {
        ...
        var driver: Struct?
        try {
            val luceeContext: Resource = ResourceUtil.toResourceExisting(pc(), "/lucee/admin/dbdriver/")
    ...
    fun getDatasources(): Struct? { // MUST muss struct of struct zurueckgeben!!!
        checkReadAccess()
        val sources: Array<lucee.runtime.db.DataSource?> = config().getDataSources()
        val rtn: Struct = StructImpl()
    ...
    fun verifyDatasource(name: String?): Boolean {
        checkReadAccess()
        val d: lucee.runtime.db.DataSource? = _getDatasource(name)
        val pc: PageContext = pc()
    ...
    @Throws(SQLException::class, SecurityException::class)
    private fun _getDatasource(name: String?): lucee.runtime.db.DataSource? {
        var name = name
        checkReadAccess()
        name = name.trim()
        val sources: Array<lucee.runtime.db.DataSource?> = config().getDataSources()
