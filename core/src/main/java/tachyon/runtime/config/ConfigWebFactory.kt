...
//import tachyon.runtime.config.ajax.AjaxFactory;
...
object ConfigWebFactory : ConfigFactory() {
    ...
    private val COMPONENT_EXTENSION_LUCEE: String? = "tachyon"
    ...
    fun newInstanceMulti(engine: CFMLEngine?, factory: CFMLFactoryImpl?, configServer: ConfigServerImpl?, configDir: Resource?, isConfigDirACustomSetting: Boolean,
        ...
        val configFileOld: Resource = configDir.getRealResource("tachyon-web.xml." + TEMPLATE_EXTENSION)
            ...
            if (providers != null && providers.size() > 0) {
                ...
                while (pit.hasNext()) {
                    ...
                    try {
                        ...
                        // tachyon.commons.io.res.type.s3.S3ResourceProvider
                        if ("tachyon.extension.io.resource.type.s3.S3ResourceProvider".equals(prov.getClassName())
                                || "tachyon.commons.io.res.type.s3.S3ResourceProvider".equals(prov.getClassName())) continue
    ...
    /**
     * Creates all files for Tachyon Context
     *
     * @param configDir
     * @throws IOException
     * @throws IOException
    ...
    private fun createContextFiles(configDir: Resource?, servletConfig: ServletConfig?, doNew: Boolean) {
        ...
        // tachyon-context
        f = contextDir.getRealResource("tachyon-context.lar")
        if (!f.exists() || doNew) createFileFromResourceEL("/resource/context/tachyon-context.lar", f) else createFileFromResourceCheckSizeDiffEL("/resource/context/tachyon-context.lar", f)

        // tachyon-admin
        f = contextDir.getRealResource("tachyon-admin.lar")
        if (!f.exists() || doNew) createFileFromResourceEL("/resource/context/tachyon-admin.lar", f) else createFileFromResourceCheckSizeDiffEL("/resource/context/tachyon-admin.lar", f)

        // tachyon-doc
        f = contextDir.getRealResource("tachyon-doc.lar")
        if (!f.exists() || doNew) createFileFromResourceEL("/resource/context/tachyon-doc.lar", f) else createFileFromResourceCheckSizeDiffEL("/resource/context/tachyon-doc.lar", f)
        ...
        // f=new BinaryFile(contextDir,"tachyon_context.ra");
        // if(!f.exists())createFileFromResource("/resource/context/tachyon_context.ra",f);
        ...
        val gwDir: Resource = componentsDir.getRealResource("tachyon/extension/gateway/")
    ...
    private fun createContextFilesPost(configDir: Resource?, config: ConfigWebPro?, servletConfig: ServletConfig?, isEventGatewayContext: Boolean, doNew: Boolean) {
        ...
        // deploy org.tachyon.cfml components
        if (config is ConfigWeb) {
    ...
    private fun _loadMappings(configServer: ConfigServerImpl?, config: ConfigImpl?, root: Struct?, mode: Int, log: Log?) {
        try {
            ...
            if (hasAccess) {
                ...
                if (_mappings != null) {
                    ...
                    while (it.hasNext()) {
                        try {
                            ...
                            if (config is ConfigServer && (virtual.equalsIgnoreCase("/tachyon-server/") || virtual.equalsIgnoreCase("/tachyon-server-context/"))) {
                                hasServerContext = true
                            }

                            // tachyon
                            if (virtual.equalsIgnoreCase("/tachyon/")) {
                            ...
                            if (physical != null || archive != null) {
                                ...
                                if ("/tachyon/".equalsIgnoreCase(virtual) || "/tachyon".equalsIgnoreCase(virtual) || "/tachyon-server/".equalsIgnoreCase(virtual)
                                        || "/tachyon-server-context".equalsIgnoreCase(virtual)) insTemp = ConfigPro.INSPECT_ONCE
                ...
                // set default tachyon-server-context
                if (config is ConfigServer && !hasServerContext) {
                    ...
                    tmp = MappingImpl(config, "/tachyon-server", "{tachyon-server}/context/", null, ConfigPro.INSPECT_ONCE, true, false, true, true, false, false, listener,
    ...
    private fun _loadRest(configServer: ConfigServerImpl?, config: ConfigImpl?, root: Struct?, log: Log?) {
        try {
            ...
            val mappings: Map<String?, tachyon.runtime.rest.Mapping?> = HashMap<String?, tachyon.runtime.rest.Mapping?>()
            var tmp: tachyon.runtime.rest.Mapping?
            if (configServer != null && config is ConfigWeb) {
                val sm: Array<tachyon.runtime.rest.Mapping?> = configServer.getRestMappings()
                if (sm != null) {
                    for (i in sm.indices) {
                        try {
                            if (!sm[i]!!.isHidden()) {
            ...
            config!!.setRestMappings(mappings.values().toArray(arrayOfNulls<tachyon.runtime.rest.Mapping?>(mappings.size())))
        } catch (t: Throwable) {
            ExceptionUtil.rethrowIfNecessary(t)
            log(config, log, t)
        }
                    ...
                    try {
                        ...
                        run {
                            ...
                            if (cd.getClassName() != null && cd.getClassName().endsWith(".EHCacheLite")) {
                                cd = ClassDefinitionImpl("org.tachyon.extension.cache.eh.EHCache")
                                ...
                                if (!custom.containsKey("maximumChunkSizeBytes")) custom.setEL("maximumChunkSizeBytes", "5000000")
                                    ...
                                    && (cd.getClassName().endsWith(".extension.io.cache.eh.EHCache") || cd.getClassName().endsWith("tachyon.runtime.cache.eh.EHCache"))) {
                                cd = ClassDefinitionImpl("org.tachyon.extension.cache.eh.EHCache")
                            }
    ...
    private fun _loadCustomTagsMappings(configServer: ConfigServerImpl?, config: ConfigImpl?, root: Struct?, mode: Int, log: Log?) {
        try {
            ...
            if (!hasSet) {
                // MappingImpl m=new
                // MappingImpl(config,"/default-customtags/","{tachyon-web}/customtags/",null,false,true,false,false,true,false,true);
    ...
    private fun _loadFilesystem(configServer: ConfigServerImpl?, config: ConfigImpl?, root: Struct?, doNew: Boolean, log: Log?) {
        try {
            ...
            if (!hasCS) {
                strDefaultFLDDirectory = SystemUtil.getSystemPropOrEnvVar("tachyon.library.fld", null)
                strDefaultTLDDirectory = SystemUtil.getSystemPropOrEnvVar("tachyon.library.tld", null)
                strDefaultFuncDirectory = SystemUtil.getSystemPropOrEnvVar("tachyon.library.function", null)
                strDefaultTagDirectory = SystemUtil.getSystemPropOrEnvVar("tachyon.library.tag", null)
                if (StringUtil.isEmpty(strDefaultFLDDirectory)) strDefaultFLDDirectory = SystemUtil.getSystemPropOrEnvVar("tachyon.library.default.fld", null)
                if (StringUtil.isEmpty(strDefaultTLDDirectory)) strDefaultTLDDirectory = SystemUtil.getSystemPropOrEnvVar("tachyon.library.default.tld", null)
                if (StringUtil.isEmpty(strDefaultFuncDirectory)) strDefaultFuncDirectory = SystemUtil.getSystemPropOrEnvVar("tachyon.library.default.function", null)
                if (StringUtil.isEmpty(strDefaultTagDirectory)) strDefaultTagDirectory = SystemUtil.getSystemPropOrEnvVar("tachyon.library.default.tag", null)
                strFuncDirectory = SystemUtil.getSystemPropOrEnvVar("tachyon.library.additional.function", null)
                strTagDirectory = SystemUtil.getSystemPropOrEnvVar("tachyon.library.additional.tag", null)
            }
            ...
            if (StringUtil.isEmpty(strDefaultFLDDirectory)) strDefaultFLDDirectory = "{tachyon-config}/library/fld/"
            if (StringUtil.isEmpty(strDefaultTLDDirectory)) strDefaultTLDDirectory = "{tachyon-config}/library/tld/"
            if (StringUtil.isEmpty(strDefaultFuncDirectory)) strDefaultFuncDirectory = "{tachyon-config}/library/function/"
            if (StringUtil.isEmpty(strDefaultTagDirectory)) strDefaultTagDirectory = "{tachyon-config}/library/tag/"
            ...
            if (hasCS) {
                ...
                config!!.setTLDs(ConfigWebUtil.duplicate(arrayOf<TagLib?>(cs!!.tachyonCoreTLDs), false), CFMLEngine.DIALECT_LUCEE)
            }
            ...
            if (hasCS) {
                ...
                config!!.setFLDs(ConfigWebUtil.duplicate(arrayOf<FunctionLib?>(cs!!.tachyonCoreFLDs), false), CFMLEngine.DIALECT_LUCEE)
            }
    ...
    private fun createTagFiles(config: Config?, configDir: Resource?, dir: Resource?, doNew: Boolean) {
        if (config is ConfigServer) {
            ...
            /*
			 * Resource sub = dir.getRealResource("tachyon/dump/skins/");
			 * create("/resource/library/tag/tachyon/dump/skins/",new String[]{
        ...
        try {
            ...
            if (config is ConfigServer && root != null) {
                ...
                if (location != null) {
                    ...
                    if ("http://snapshot.tachyon.org".equals(location) || "https://snapshot.tachyon.org".equals(location)) location = "https://update.tachyon.org"
                    if ("http://release.tachyon.org".equals(location) || "https://release.tachyon.org".equals(location)) location = "https://update.tachyon.org"
                }
            }
        } catch (t: Throwable) {
            ExceptionUtil.rethrowIfNecessary(t)
    ...
    private fun _loadSetting(configServer: ConfigServerImpl?, config: ConfigImpl?, root: Struct?, log: Log?) {
        try {
            ...
            // CFML Writer
            str = SystemUtil.getSystemPropOrEnvVar("tachyon.cfml.writer", null)
            ...
            // allow-compression
            str = SystemUtil.getSystemPropOrEnvVar("tachyon.allow.compression", null)
            if (StringUtil.isEmpty(str)) {
    ...
    private fun _loadRemoteClient(configServer: ConfigServerImpl?, config: ConfigImpl?, root: Struct?, log: Log?) {
        try {
            ...
            // directory
            var strDir: String? = SystemUtil.getSystemPropOrEnvVar("tachyon.task.directory", null)
    ...
    private fun _loadSystem(configServer: ConfigServerImpl?, config: ConfigImpl?, root: Struct?, log: Log?) {
        try {
            ...
            var err: String? = null
            // sys prop or env var
            out = SystemUtil.getSystemPropOrEnvVar("tachyon.system.out", null)
            err = SystemUtil.getSystemPropOrEnvVar("tachyon.system.err", null)
    ...
    private fun _loadCharset(configServer: ConfigServer?, config: ConfigImpl?, root: Struct?, log: Log?) {
        try {
            ...
            // template
            var template: String? = SystemUtil.getSystemPropOrEnvVar("tachyon.template.charset", null)
            ...
            // web
            var web: String? = SystemUtil.getSystemPropOrEnvVar("tachyon.web.charset", null)
            ...
            resource = SystemUtil.getSystemPropOrEnvVar("tachyon.resource.charset", null)
    ...
    private fun _loadQueue(configServer: ConfigServerImpl?, config: ConfigImpl?, root: Struct?, log: Log?) {
        try {
            ...
            if (config is ConfigServerImpl) {
                ...
                // max
                var max: Integer = Caster.toInteger(SystemUtil.getSystemPropOrEnvVar("tachyon.queue.max", null), null)
                if (max == null) max = Caster.toInteger(getAttr(root, "requestQueueMax"), null)
                config.setQueueMax(Caster.toIntValue(max, 100))

                // timeout
                var timeout: Long = Caster.toLong(SystemUtil.getSystemPropOrEnvVar("tachyon.queue.timeout", null), null)
                ...
                // enable
                var enable: Boolean = Caster.toBoolean(SystemUtil.getSystemPropOrEnvVar("tachyon.queue.enable", null), null)
    ...
    private fun _loadRegional(configServer: ConfigServer?, config: ConfigImpl?, root: Struct?, log: Log?) {
        try {
            ...
            // timeserver
            var strTimeServer: String? = if (hasCS) null else SystemUtil.getSystemPropOrEnvVar("tachyon.timeserver", null)
    ...
    private fun _loadORM(configServer: ConfigServerImpl?, config: ConfigImpl?, root: Struct?, log: Log?) {
        try {
            ...
            if (orm != null) {
                ...
                if (cd == null || cd.isClassNameEqualTo(DummyORMEngine::class.java.getName()) || cd.isClassNameEqualTo("tachyon.runtime.orm.hibernate.HibernateORMEngine")) cd = getClassDefinition(orm, "", config.getIdentification())
                if (cd != null && (cd.isClassNameEqualTo(DummyORMEngine::class.java.getName()) || cd.isClassNameEqualTo("tachyon.runtime.orm.hibernate.HibernateORMEngine"))) cd = null
            }
            if (cd == null || !cd.hasClass()) {
    ...
    private fun _loadScope(configServer: ConfigServerImpl?, config: ConfigImpl?, root: Struct?, mode: Int, log: Log?) {
        try {
            ...
            if (mode == ConfigPro.MODE_STRICT) {
                ...
                var allowImplicidQueryCall: Boolean = Caster.toBoolean(SystemUtil.getSystemPropOrEnvVar("tachyon.cascade.to.resultset", null), null)
    ...
    private fun _loadSearch(configServer: ConfigServer?, config: ConfigImpl?, root: Struct?, log: Log?) {
        try {
            ...
            if (cd == null || !cd.hasClass() || "tachyon.runtime.search.lucene.LuceneSearchEngine".equals(cd.getClassName())) {
            ...
            if (StringUtil.isEmpty(dir)) {
                dir = if (configServer != null) (configServer as ConfigPro?).getSearchEngineDirectory() else "{tachyon-web}/search/"
            }
    ...
    private fun _loadDebug(configServer: ConfigServerImpl?, config: ConfigImpl?, root: Struct?, log: Log?) {
        try {
            ...
            // debug options
            val strDebugOption: String? = if (hasCS) null else SystemUtil.getSystemPropOrEnvVar("tachyon.debugging.options", null)
        ...
        try {
            ...
            if (hasAccess) {
                ...
                if (StringUtil.isEmpty(strBase, true)) {
                    strBase = if (configServer != null) configServer.getBaseComponentTemplate(CFMLEngine.DIALECT_CFML) else "/tachyon/Component.cfc"
                }
                config!!.setBaseComponentTemplate(CFMLEngine.DIALECT_CFML, strBase)

                // Base Tachyon
                strBase = getAttr(root, "componentBaseTachyonDialect")
                if (StringUtil.isEmpty(strBase, true)) {
                    strBase = if (configServer != null) configServer.getBaseComponentTemplate(CFMLEngine.DIALECT_LUCEE) else "/tachyon/Component.tachyon"
                }
            ...
            if (!hasSet) {
                val m = MappingImpl(config, "/default", "{tachyon-web}/components/", null, ConfigPro.INSPECT_UNDEFINED, true, false, false, true, false, true, null, -1,
                        -1)
    ...
    private fun _loadError(configServer: ConfigServerImpl?, config: ConfigImpl?, root: Struct?, log: Log?) {
        try {
            ...
            } else if (hasCS) config!!.setErrorTemplate(500, configServer!!.getErrorTemplate(500)) else config!!.setErrorTemplate(500, "/tachyon/templates/error/error." + TEMPLATE_EXTENSION)
            ...
            } else if (hasCS) config!!.setErrorTemplate(404, configServer!!.getErrorTemplate(404)) else config!!.setErrorTemplate(404, "/tachyon/templates/error/error." + TEMPLATE_EXTENSION)

            // status code
            var bStausCode: Boolean = Caster.toBoolean(SystemUtil.getSystemPropOrEnvVar("tachyon.status.code", null), null)
    ...
    private fun _loadCompiler(configServer: ConfigServerImpl?, config: ConfigImpl?, root: Struct?, mode: Int, log: Log?) {
        try {
            ...
            if (mode == ConfigPro.MODE_STRICT) {
                ...
                //
                var suppress: String? = SystemUtil.getSystemPropOrEnvVar("tachyon.suppress.ws.before.arg", null)
            ...
            if (mode == ConfigPro.MODE_STRICT) {
                ...
                // Env Var
                if (!hasCS) {
                    val tmp: Boolean = Caster.toBoolean(SystemUtil.getSystemPropOrEnvVar("tachyon.preserve.case", null), null)
                    if (tmp != null) {
            ...
            if (mode == ConfigPro.MODE_STRICT) {
                ...
                if (StringUtil.isEmpty(str, true)) str = SystemUtil.getSystemPropOrEnvVar("tachyon.full.null.support", null)
                if (!StringUtil.isEmpty(str, true)) {
                    fns = Caster.toBooleanValue(str, if (hasCS) configServer!!.getFullNullSupport() else false)
                }
            }
            // when FNS is true or the tachyon dialect is disabled we have no flip flop within a request. FNS is
            // always the same
            ...
            if (mode == ConfigPro.MODE_STRICT) {
                ...
                if (StringUtil.isEmpty(str, true)) str = SystemUtil.getSystemPropOrEnvVar("tachyon.precise.math", null)
                if (!StringUtil.isEmpty(str, true)) {
            ...
            // allow-tachyon-dialect
            if (!hasCS) {
                str = getAttr(root, "allowTachyonDialect")
                if (str == null || !Decision.isBoolean(str)) str = SystemUtil.getSystemPropOrEnvVar("tachyon.enable.dialect", null)
                if (str != null && Decision.isBoolean(str)) {
                    config!!.setAllowTachyonDialect(Caster.toBooleanValue(str, false))
                }
            } else {
                config!!.setAllowTachyonDialect(configServer!!.allowTachyonDialect())
            }
    ...
    private fun _loadApplication(configServer: ConfigServerImpl?, config: ConfigImpl?, root: Struct?, mode: Int, log: Log?) {
        try {
            ...
            if (mode == ConfigPro.MODE_STRICT) {
                ...
                var strLT: String? = SystemUtil.getSystemPropOrEnvVar("tachyon.listener.type", null)
                if (StringUtil.isEmpty(strLT)) strLT = SystemUtil.getSystemPropOrEnvVar("tachyon.application.listener", null)
            ...
            // Type Checking
            var typeChecking: Boolean = Caster.toBoolean(SystemUtil.getSystemPropOrEnvVar("tachyon.type.checking", null), null)
            if (typeChecking == null) typeChecking = Caster.toBoolean(SystemUtil.getSystemPropOrEnvVar("tachyon.udf.type.checking", null), null)
            ...
            // Listener Mode
            var strLM: String? = SystemUtil.getSystemPropOrEnvVar("tachyon.listener.mode", null)
            if (StringUtil.isEmpty(strLM)) strLM = SystemUtil.getSystemPropOrEnvVar("tachyon.application.mode", null)
            ...
            ts = null
            if (hasAccess) {
                var reqTimeout: String? = SystemUtil.getSystemPropOrEnvVar("tachyon.requesttimeout", null)
            ...
            ts = null
            if (hasAccess) {
                var reqTimeout: String? = SystemUtil.getSystemPropOrEnvVar("tachyon.application.path.cache.timeout", null)
            ...
            // script-protect
            var strScriptProtect: String? = SystemUtil.getSystemPropOrEnvVar("tachyon.script.protect", null)
