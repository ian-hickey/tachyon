...
class CFMLEngineImpl private constructor(factory: CFMLEngineFactory?, bc: BundleCollection?) : CFMLEngine {
        ...
        fun getInstance(factory: CFMLEngineFactory?, bc: BundleCollection?): CFMLEngine? {
            if (engine == null) {
                if (SystemUtil.getLoaderVersion() < 6.0) {
                    ...
                    if (SystemUtil.isWindows()) throw RuntimeException("You need to update a newer lucee.jar to run this version, you can download the latest jar from https://download.lucee.org.") else if (SystemUtil.getLoaderVersion() < 5.8) throw RuntimeException("You need to update your lucee.jar to run this version, you can download the latest jar from https://download.lucee.org.") else if (SystemUtil.getLoaderVersion() < 5.9) LogUtil.log(Log.LEVEL_INFO, "startup",
    ...
    private var luceeScriptEngine: ScriptEngineFactory? = null
    private var luceeTagEngine: ScriptEngineFactory? = null
    ...
    private fun deployBundledExtension(cs: ConfigServerImpl?, validate: Boolean): Int {
        ...
        try {
            ...
            // + "]");
            val names: Array<String?> = lucee.runtime.type.util.ListUtil.listToStringArray(index, ';')
            var name: String?
    ...
    fun addServletConfig(config: ServletConfig?) {
        if (PageSourceImpl.logAccessDirectory == null) {
            val str: String = config.getInitParameter("lucee-log-access-directory")
    ...
    private fun addEventListener(sc: ServletContext?) {
        ...
        LogUtil.log(configServer, Log.LEVEL_INFO, "startup", "Lucee was not able to register an event listener with " + if (sc == null) "null" else sc.getClass().getName())
    }
    ...
    private fun getConfigDirectory(sg: ServletConfig?, configServer: ConfigServerImpl?, countExistingContextes: Int, isCustomSetting: RefBoolean?): Resource? {
        ...
        if (countExistingContextes > 1 && strConfig != null && strConfig.indexOf('{') === -1) {
            val text = "Static path [$strConfig] for servlet init param [lucee-web-directory] is not allowed, path must use a web-context specific placeholder."
        ...
        if (!configDir.exists() || ResourceUtil.isEmptyDirectory(configDir, null)) {
            ...
            if (configDir.getName().equals("lucee") && configDir.getParentResource().getRealResource("railo").also { railoRoot = it }.isDirectory()) {
                try {
    ...
    private fun _service(servlet: HttpServlet?, req: HttpServletRequest?, rsp: HttpServletResponse?, type: Short) {
        ...
        if (type == Request.TYPE_LUCEE) {
            if (!(factory.getConfig() as ConfigPro).allowLuceeDialect()) {
                try {
    ...
    @Override
    fun getResourceUtil(): lucee.runtime.util.ResourceUtil? {
        return ResourceUtilImpl.getInstance()
    }

    @Override
    fun getHTTPUtil(): lucee.runtime.util.HTTPUtil? {
    ...
    fun isRunning(): Boolean {
        try {
            ...
            if (other !== this && controlerState!!.active() && other !is CFMLEngineWrapper) {
                LogUtil.log(configServer, Log.LEVEL_INFO, "startup", "CFMLEngine is still set to true but no longer valid, " + lucee.runtime.config.Constants.NAME.toString() + " disable this CFMLEngine.")
                controlerState!!.setActive(false)
                reset()
                return false
            }
    ...
    fun cli(config: Map<String?, String?>?, servletConfig: ServletConfig?) {
        ...
        val uri: URI
        uri = try {
            lucee.commons.net.HTTPUtil.toURI(strUri)
    ...
    fun getScriptEngineFactory(dialect: Int): ScriptEngineFactory? {
        ...
        if (luceeScriptEngine == null) luceeScriptEngine = ScriptEngineFactoryImpl(this, false, dialect)
    ...
    fun getTagEngineFactory(dialect: Int): ScriptEngineFactory? {
        ...
        if (luceeTagEngine == null) luceeTagEngine = ScriptEngineFactoryImpl(this, true, dialect)
    ...
    @Override
    fun getSystemUtil(): lucee.runtime.util.SystemUtil? {
    ...
    fun onStart(config: ConfigPro?, reload: Boolean) {
        ...
        if ((isWeb || config.getAdminMode() === ConfigImpl.ADMINMODE_SINGLE)
            ...
            val msg = "Lucee warmup completed. Shutting down."
        ...
        val listenerTemplateCFML: Resource = config.getConfigDir().getRealResource("context/" + context + "." + lucee.runtime.config.Constants.getCFMLComponentExtension())
        val listenerTemplateLucee: Resource = config.getConfigDir().getRealResource("context/" + context + "." + lucee.runtime.config.Constants.getLuceeComponentExtension())
        var listenerTemplateCFMLWebRoot: Resource? = null
        var listenerTemplateLuceeWebRoot: Resource? = null
        if (isWeb) {
            try {
    ...
    private inner class OnStart(config: ConfigPro?, dialect: Int, context: String?, reload: Boolean, inWebRoot: Boolean) : Thread() {
        ...
        fun run() {
            ...
            val requestURI = ((if (inWebRoot) "" else "/" + if (isWeb) "lucee" else "lucee-server") + "/" + context + "."
                    + if (dialect == CFMLEngine.DIALECT_LUCEE) lucee.runtime.config.Constants.getLuceeComponentExtension() else lucee.runtime.config.Constants.getCFMLComponentExtension())
    ...
    init {
        ...
        bundleCollection = bc
        allowRequestTimeout = Caster.toBooleanValue(SystemUtil.getSystemPropOrEnvVar("lucee.requesttimeout", null), true)
        ...
        val installExtensions: Boolean = Caster.toBooleanValue(SystemUtil.getSystemPropOrEnvVar("lucee.extensions.install", null), true)
