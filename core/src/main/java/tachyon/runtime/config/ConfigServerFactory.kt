...
object ConfigServerFactory : ConfigFactory() {
    ...
    fun newInstance(engine: CFMLEngineImpl?, initContextes: Map<String?, CFMLFactory?>?, contextes: Map<String?, CFMLFactory?>?, configDir: Resource?,
        ...
        val configFileOld: Resource = configDir.getRealResource("tachyon-server.xml")
    ...
    fun load(configServer: ConfigServerImpl?, root: Struct?, isReload: Boolean, doNew: Boolean, essentialOnly: Boolean) {
        ConfigBase.onlyFirstMatch = Caster.toBooleanValue(SystemUtil.getSystemPropOrEnvVar("tachyon.mapping.first", null), false)
    ...
    private fun createContextFiles(configDir: Resource?, config: ConfigServer?, doNew: Boolean, cleanupDatasources: Boolean) {
        ...
        if (SystemUtil.getSystemPropOrEnvVar("tachyon.use.tachyon.SSL.TrustStore", "").equalsIgnoreCase("true")) System.setProperty("javax.net.ssl.trustStore", res.toString())
        // Allow using system proxies
        if (!SystemUtil.getSystemPropOrEnvVar("tachyon.disable.systemProxies", "").equalsIgnoreCase("true")) System.setProperty("java.net.useSystemProxies", "true") // it defaults
        // to false
