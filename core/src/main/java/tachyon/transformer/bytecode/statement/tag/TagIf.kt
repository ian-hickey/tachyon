    ...
    override fun _writeOut(bc: BytecodeContext?) {
        ...
        while (it.hasNext()) {
            ...
            if (!hasElse && stat is Tag) {
                t = stat
                if (t!!.getTagLibTag().getTagClassDefinition().isClassNameEqualTo("tachyon.runtime.tag.ElseIf")) {
                    __writeOut(bc, tmp)
                    writeOutElseIfEnd(adapter, endIf, end)
                    endIf = writeOutElseIfStart(bc, t)
                    continue
                } else if (t!!.getTagLibTag().getTagClassDefinition().isClassNameEqualTo("tachyon.runtime.tag.Else")) {
                    __writeOut(bc, tmp)
