...
class LFunctionValue : RefSupport, Ref {
    ...
    fun getValue(pc: PageContext?): Object? {
        ...
        if (name is Set) {
            return FunctionValueImpl(lucee.runtime.type.util.ListUtil.arrayToList(toStringArray(pc, name as Set?), "."), if (refValue == null) objValue else refValue.getValue(pc))
        }
