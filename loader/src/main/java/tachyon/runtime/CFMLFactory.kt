...
abstract class CFMLFactory : JspFactory() {
    ...
    /**
     * similar to getPageContext Method but return the concrete implementation of the Tachyon PageContext
     ...
     */
    @Deprecated
    abstract fun getTachyonPageContext(servlet: HttpServlet?, req: HttpServletRequest?, rsp: HttpServletResponse?, errorPageURL: String?, needsSession: Boolean, bufferSize: Int,
     ...
     */
    @Deprecated
    @Deprecated("use instead <code>releaseTachyonPageContext(PageContext pc, boolean unregister)</code>")
    abstract fun releaseTachyonPageContext(pc: PageContext?)
     ...
     */
    abstract fun releaseTachyonPageContext(pc: PageContext?, unregister: Boolean)
    ...
    abstract fun getTachyonExtensions(): Iterator<String?>?
}