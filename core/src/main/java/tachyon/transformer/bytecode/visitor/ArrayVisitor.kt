...
class ArrayVisitor {
    fun visitBegin(adapter: GeneratorAdapter?, type: Type?, length: Int) {
        adapter.push(length)
        adapter.newArray(type)
    }

    fun visitBeginItem(adapter: GeneratorAdapter?, index: Int) {
        adapter.dup()
        adapter.push(index)
    }
	 ...
	 */
    fun visitEndItem(adapter: GeneratorAdapter?) {
