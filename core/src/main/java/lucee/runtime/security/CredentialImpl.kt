...
class CredentialImpl(var username: String?, var password: String?, var roles: Array<String?>?, rolesDir: Resource?, privateKey: String?, salt: String?, iter: Int) : Credential {
    ...
    companion object {
        ...
        init {
            // salt
            val tmp: String = SystemUtil.getSystemPropOrEnvVar("lucee.loginstorage.salt", null)
            if (StringUtil.isEmpty(lucee.runtime.security.tmp, true)) lucee.runtime.security.tmp = "nkhuvghc" else lucee.runtime.security.tmp = lucee.runtime.security.tmp.trim()
            staticSalt = toSalt(lucee.runtime.security.tmp)

            // salt iteration
            val itmp: Int = Caster.toIntValue(SystemUtil.getSystemPropOrEnvVar("lucee.loginstorage.iterations", null), 0)
            if (lucee.runtime.security.itmp < 1) lucee.runtime.security.itmp = 10
            staticIter = lucee.runtime.security.itmp

            // private key
            lucee.runtime.security.tmp = SystemUtil.getSystemPropOrEnvVar("lucee.loginstorage.privatekey", null)
            if (!StringUtil.isEmpty(lucee.runtime.security.tmp, true)) {
                staticPrivateKey = lucee.runtime.security.tmp.trim()
