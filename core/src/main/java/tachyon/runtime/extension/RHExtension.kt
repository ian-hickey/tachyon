...
class RHExtension : Serializable {
    ...
    private fun readManifestConfig(manifest: Manifest?, label: String?, _img: String?) {
        ...
        readCoreVersion(label, StringUtil.unwrap(attr.getValue("tachyon-core-version")), info)
        readLoaderVersion(label, StringUtil.unwrap(attr.getValue("tachyon-loader-version")))
    ...
    private fun readManifestConfig(id: String?, data: Struct?, label: String?, _img: String?) {
        ...
        readCategories(label, cat)
        readCoreVersion(label, ConfigWebFactory.getAttr(data, "tachyonCoreVersion", "tachyon-core-version"), info)
        readLoaderVersion(label, ConfigWebFactory.getAttr(data, "tachyonLoaderVersion", "tachyon-loader-version"))
    ...
    private fun readLoaderVersion(label: String?, str: String?) {
		 ...
		 * " Loader Version must be at least [" + str + "], update the Tachyon.jar first."); }
		 */
    }
    ...
    fun populate(el: Struct?, full: Boolean) {
        ...
        // core version
        if (minCoreVersion != null) el.setEL("tachyonCoreVersion", toStringForAttr(minCoreVersion.toString())) else el.removeEL(KeyImpl.init("tachyonCoreVersion"))
