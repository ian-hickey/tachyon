...
object GetFunctionList : Function {
    ...
    @Throws(PageException::class)
    fun call(pc: PageContext?, strDialect: String?): lucee.runtime.type.Struct? {
        val dialect: Int = ConfigWebUtil.toDialect(strDialect, -1)
        if (dialect == -1) throw FunctionException(pc, "GetFunctionList", 1, "dialect", "value [$strDialect] is invalid, valid values are [cfml,lucee]")
        return _call(pc, dialect)
    }

    @Throws(PageException::class)
    private fun _call(pc: PageContext?, dialect: Int): lucee.runtime.type.Struct? {
