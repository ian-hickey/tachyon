...
class CFMLTransformer @JvmOverloads constructor(private val codeIsland: Boolean = false) {
    ...
    fun transform(factory: Factory?, config: ConfigPro?, ps: PageSource?, tlibs: Array<TagLib?>?, flibs: Array<FunctionLib?>?, returnValue: Boolean, ignoreScopes: Boolean): Page? {
        ...
        if (p.isPage()) {
            if (isCFML) possibleUndetectedComponent = isCFMLCompExt else if (Constants.isTachyonComponentExtension(ResourceUtil.getExtension(ps.getResource(), ""))) {
                var expr: Expression?
                var stat: Statement
                var po: PrintOut
                var ls: LitString?
                ...
                if (tagLibTag.isTagDependent()) {
                    ...
                    while (true) {
                        ...
                        if (tagLibEnd != null) {
                            ...
                            // tachyon.print.ln(data.cfml.getLine()+" - "+data.cfml.getColumn()+" -
