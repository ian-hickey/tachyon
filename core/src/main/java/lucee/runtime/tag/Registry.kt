...
class Registry : TagImpl() {
    ...
    private fun doGetAll() {
        ...
        try {
            ...
            if (entries != null) {
                val qry: lucee.runtime.type.Query = QueryImpl(arrayOf<String?>("entry", "type", "value"), arrayOf<String?>("VARCHAR", "VARCHAR", "OTHER"), entries.size, "query")
                ...
                if (sort != null) {
                    ...
                    for (i in arr.indices.reversed()) {
                        ...
                        if (col.size == 1) qry.sort(KeyImpl.init(col[0].trim())) else if (col.size == 2) {
                            ...
                            if (order.equals("asc")) qry.sort(KeyImpl.init(col[0]), lucee.runtime.type.Query.ORDER_ASC) else if (order.equals("desc")) qry.sort(KeyImpl.init(col[0]), lucee.runtime.type.Query.ORDER_DESC) else throw ApplicationException("invalid order type [" + col[1] + "]")
                        }
                    }
                }
