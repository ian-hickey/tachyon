...
class CFMLEngineFactory protected constructor(config: ServletConfig) : CFMLEngineFactorySupport() {
    ...
    fun readInitParam(config: ServletConfig) {
        ...
        var initParam: String? = config.getInitParameter("lucee-server-directory")
    ...
    private fun initEngine() {
        ...
        if (patches != null) {
            for (patch in patches) {
                if (patch.getName().startsWith("tmp.lco")) patch.delete() else if (patch.lastModified() < coreCreated) patch.delete() else if (patch.length() < 1000000L) patch.delete() else if (lucee == null || Util.isNewerThan(toVersion(patch.getName(), lucee.loader.engine.CFMLEngineFactory.Companion.VERSION_ZERO), toVersion(lucee.getName(), lucee.loader.engine.CFMLEngineFactory.Companion.VERSION_ZERO))) lucee = patch
            }
        }
        if (lucee != null && Util.isNewerThan(coreVersion, toVersion(lucee.getName(), lucee.loader.engine.CFMLEngineFactory.Companion.VERSION_ZERO))) lucee = null
            ...
            if (lucee == null) {
                ...
                try {
                    ...
                    if (`is` == null) {
                        ...
                        val s: String = System.getProperty("lucee.core.path")
                        if (s != null) {
                            val dir = File(s)
                ...
                if (rc.exists()) {
                    lucee = File(patcheDir, lucee.loader.engine.CFMLEngineFactory.Companion.getVersion(rc).toString() + "." + coreExt)
                    try {
                        `is` = FileInputStream(rc)
                        os = BufferedOutputStream(FileOutputStream(lucee))
                        copy(`is`, os)
                    } finally {
                        closeEL(`is`)
                        closeEL(os)
                ...
                lucee.loader.engine.CFMLEngineFactory.Companion.setEngine(engine)
            } else {
                bundleCollection = BundleLoader.loadBundles(this, felixCacheDirectory, bundleDirectory, lucee, bundleCollection)
                // bundle=loadBundle(lucee);
                log(Logger.LOG_DEBUG, "Loaded bundle: [" + bundleCollection.core.getSymbolicName().toString() + "]")
                lucee.loader.engine.CFMLEngineFactory.Companion.setEngine(getEngine(bundleCollection))
                log(Logger.LOG_DEBUG, "Loaded engine: [" + lucee.loader.engine.CFMLEngineFactory.Companion.singelton + "]")
            }
            version = lucee.loader.engine.CFMLEngineFactory.Companion.singelton.getInfo().getVersion()
            log(Logger.LOG_DEBUG, "Loaded Lucee Version [" + lucee.loader.engine.CFMLEngineFactory.Companion.singelton.getInfo().getVersion().toString() + "]")
        ...
        // check updates
        var updateType: String = lucee.loader.engine.CFMLEngineFactory.Companion.singelton.getUpdateType()
        if (updateType == null || updateType.length() === 0) updateType = "manuell" // TODO should be manual?
        if (updateType.equalsIgnoreCase("auto")) lucee.loader.engine.CFMLEngineFactory.UpdateChecker(this, null).start()
    }
    ...
    private fun _update(id: Identification): Boolean {
        val newLucee: File = downloadCore(id) ?: return false
        if (singelton != null) singelton.reset()
        val v: Version? = null
        try {
            bundleCollection = BundleLoader.loadBundles(this, felixCacheDirectory, bundleDirectory, newLucee, bundleCollection)
    ...
    fun downloadBundle(symbolicName: String, symbolicVersion: String, id: Identification?): File? {
        ...
        val str: String = Util._getSystemPropOrEnvVar("lucee.enable.bundle.download", null)
        if (str != null && ("false".equalsIgnoreCase(str) || "no".equalsIgnoreCase(str))) { // we do not use CFMLEngine to cast, because the engine may not exist yet
            throw RuntimeException("Lucee is missing the Bundle jar, " + symbolicName + ":" + symbolicVersion
    ...
    private fun deployBundledBundle(bundleDirectory: File?, symbolicName: String, symbolicVersion: String): File? {
        ...
        if (`is` != null) log(Logger.LOG_DEBUG, "Found ]/bundles/$osgiFileName] in lucee.jar") else log(Logger.LOG_INFO, "Could not find [/bundles/$osgiFileName] in lucee.jar")
        if (`is` == null) {
            ...
            isPack200 = true
            if (`is` != null) log(Logger.LOG_DEBUG, "Found [/bundles/$osgiFileName$pack20Ext] in lucee.jar") else log(Logger.LOG_INFO, "Could not find [/bundles/$osgiFileName$pack20Ext] in lucee.jar")
        }
        if (`is` != null) {
            var temp: File? = null
            try {
 ...
 - available Version [$strAvailableVersion]""")
        ...
        val newLucee = File(patchDir, "$strAvailableVersion.lco")
        ////
        var code: Int
        var conn: HttpURLConnection
        try {
        ...
        if (code != 200) {
            ...
            // no download available!
            if (code != 200) {
                val msg = ("Lucee failed to download the core for version [" + version.toString().toString() + "] from " + updateUrl.toString() + ", please download it manually and copy to ["
                        + patchDir.toString() + "]")
                log(Logger.LOG_ERROR, msg)
                conn.disconnect()
                throw IOException(msg)
        ...
        if (newLucee.createNewFile()) {
            copy(conn.getContent() as InputStream, FileOutputStream(newLucee))
            conn.disconnect()

            // when it is a loader extract the core from it
            val tmp: File? = extractCoreIfLoader(newLucee)
            if (tmp != null) {
    ...
    val updateLocation: URL?
        get() {
            ...
            if (location == null) {
                ...
                try {
                    val xml = File(getResourceRoot(), "context/lucee-server.xml")
    ...
    val patchDirectory: File?
        get() {
            var pd: File? = getDirectoryByPropOrEnv("lucee.patches.dir")
    ...
    val bundleDirectory: File?
        get() {
            var bd: File? = getDirectoryByPropOrEnv("lucee.bundles.dir")
    ...
    fun getResourceRoot(): File? {
        if (resourceRoot == null) {
            resourceRoot = File(_getResourceRoot(), "lucee-server")
    ...
    private fun _getResourceRoot(): File? {
        ...
        val lbd: File? = getDirectoryByPropOrEnv("lucee.base.dir") // directory defined by the caller
        var root: File? = lbd
        ...
        val modernDir = File(root, "lucee-server")
        if (true) {
            // there is a server context in the old lucee location, move that one
            var classicDir: File
            log(Logger.LOG_DEBUG, "classic-root-directory:$classicRoot")
            var had = false
            if (classicRoot.isDirectory() && File(classicRoot, "lucee-server").also { classicDir = it }.isDirectory()) {
    ...
    private fun getEngine(bc: BundleCollection?): CFMLEngine {
        ...
        val clazz: Class<*> = bc.core.loadClass("lucee.runtime.engine.CFMLEngineImpl")
    ...
    companion object {
        ...
        private const val UPDATE_LOCATION = "https://update.lucee.org" // MUST from server.xml
        ...
        private var luceeServerRoot: File? = null
        ...
        val instance: lucee.loader.engine.CFMLEngine?
            get() {
        ...
        fun _extractCoreIfLoader(file: File?): File? {
            ...
            return try {
                ...
                val trg: File = File.createTempFile("lucee", ".lco")
        ...
        private fun copyRecursiveAndRename(src: File, trg: File) {
            ...
            if (src.isDirectory()) {
                ...
                if (trg.getName().equals("railo-server.xml")) {
                    trg = File(trg.getParentFile(), "lucee-server.xml")
        ...
        fun getClassLoaderRoot(cl: ClassLoader): File? {
            val path = "lucee/loader/engine/CFMLEngine.class"
