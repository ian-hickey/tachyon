...
import tachyon.runtime.util.HTMLUtil
 ...
 */
object HTMLEntities {
    val HTMLV20: Short = HTMLUtil.HTMLV20
    val HTMLV32: Short = HTMLUtil.HTMLV32
    val HTMLV40: Short = HTMLUtil.HTMLV40

    // Basic
    private const val OFFSET_BASIC = 34
    private val BASIC_ARRAY = arrayOfNulls<String>(63 - OFFSET_BASIC)

    // HTML 32
    private const val OFFSET_ISO8859_1 = 160
    private val ISO8859_1_ARRAY = arrayOfNulls<String>(256 - OFFSET_ISO8859_1)

    // HTML 40 (1)
    private const val OFFSET_HTML40_1 = 402
    private val HTML40_1_ARRAY = arrayOfNulls<String>(403 - OFFSET_HTML40_1)

    // HTML 40 (2)
    private const val OFFSET_HTML40_2 = 913
    private val HTML40_2_ARRAY = arrayOfNulls<String>(983 - OFFSET_HTML40_2)

    // HTML 40 (3)
    private const val OFFSET_HTML40_3 = 338
    private val HTML40_3_ARRAY = arrayOfNulls<String>(377 - OFFSET_HTML40_3)

    // HTML 40 (4)
    private const val OFFSET_HTML40_4 = 710
    private val HTML40_4_ARRAY = arrayOfNulls<String>(733 - OFFSET_HTML40_4)

    // HTML 40 (5)
    private const val OFFSET_HTML40_5 = 8194
    private val HTML40_5_ARRAY = arrayOfNulls<String>(9831 - OFFSET_HTML40_5)

    // HTML 20
    private val HTML20_DATA = arrayOf(BASIC_ARRAY)
    private val HTML20_OFFSET = intArrayOf(OFFSET_BASIC)

    // HTML 32
    private val HTML32_DATA = arrayOf(BASIC_ARRAY, ISO8859_1_ARRAY)
    private val HTML32_OFFSET = intArrayOf(OFFSET_BASIC, OFFSET_ISO8859_1)

    // HTML 40
    private val HTML40_DATA = arrayOf(BASIC_ARRAY, ISO8859_1_ARRAY, HTML40_1_ARRAY, HTML40_2_ARRAY, HTML40_3_ARRAY, HTML40_4_ARRAY, HTML40_5_ARRAY)
    private val HTML40_OFFSET = intArrayOf(OFFSET_BASIC, OFFSET_ISO8859_1, OFFSET_HTML40_1, OFFSET_HTML40_2, OFFSET_HTML40_3, OFFSET_HTML40_4, OFFSET_HTML40_5)
    ...
    fun escapeHTML(str: String, version: Short): String {
        ...
        val offset: IntArray
        val rtn = StringBuilder(str.length())
        ...
        outer@ for (i in chars.indices) {
            val c = chars[i]
            if (c == CR) continue  // for compatibility to ACF
            for (y in offset.indices) {
                if (c >= offset[y].toChar() && c.toInt() < data[y].length + offset[y]) {
                    val replacement = data[y][(c - offset[y]).toInt()]
                    if (replacement != null) {
                        rtn.append('&')
    ...
    private fun unescapeHTMLEntity(str: String): String {
        val ranges = HTML40_DATA
        val offset = HTML40_OFFSET

        // Number Entity
        if (str.indexOf('#') === 0) {
            return if (str.length() === 1) "&$str;" else try {
                (Integer.parseInt(str.substring(1)) as Char).toString() + ""
        ...
        for (i in ranges.indices) {
            ...
            for (y in range.indices) {
                ...
                if (el != null && el.equalsIgnoreCase(str)) {
                    return ((y + offset[i]) as Char).toString() + ""
                }
            }
        }
        // }
