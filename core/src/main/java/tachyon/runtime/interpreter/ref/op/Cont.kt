 ...
 * imp operation
 */
class Cont(cont: Ref?, left: Ref?, right: Ref?, limited: Boolean) : RefSupport(), Ref {
    private val cont: Ref?
    ...
    fun getValue(pc: PageContext?): Object? {
        ...
        return if (Caster.toBooleanValue(cont.getValue(pc))) left.getValue(pc) else right.getValue(pc)
    }
