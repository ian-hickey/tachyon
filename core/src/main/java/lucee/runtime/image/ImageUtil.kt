...
object ImageUtil {
    private fun getImageClass(): Class? {
        return try {
            ...
            ClassUtil.loadClassByBundle("org.lucee.extension.image.Image", "image.extension", null as Version?, id, null)
        } catch (e: Exception) {
            null
        }
    }
    ...
    private fun getImageUtilClass(): Class? {
        return try {
            val config: Config = ThreadLocalPageContext.getConfig()
            val id: Identification? = if (config == null) null else config.getIdentification()
            ClassUtil.loadClassByBundle("org.lucee.extension.image.ImageUtil", "image.extension", null as Version?, id, null)
        } catch (e: Exception) {
            null
        }
    }
