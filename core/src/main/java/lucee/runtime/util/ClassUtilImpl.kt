...
class ClassUtilImpl : ClassUtil {
    ...
    fun loadBIF(pc: PageContext?, name: String?): BIF? {
        ...
        val res: Class<*> = lucee.commons.lang.ClassUtil.loadClass(name, null)
        if (res != null) {
            return if (Reflector.isInstaneOf(res, BIF::class.java, false)) {
                try {
                    lucee.commons.lang.ClassUtil.newInstance(res) as BIF
                } catch (e: Exception) {
    ...
    fun loadBIF(pc: PageContext?, name: String?, bundleName: String?, bundleVersion: Version?): BIF? {
        // first of all we chek if itis a class
        val res: Class<*> = lucee.commons.lang.ClassUtil.loadClassByBundle(name, bundleName, bundleVersion, pc.getConfig().getIdentification(),
