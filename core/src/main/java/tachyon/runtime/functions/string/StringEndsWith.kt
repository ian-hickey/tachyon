 ...
 */
class StringEndsWith : BIF() {
    @Override
    @Throws(PageException::class)
    operator fun invoke(pc: PageContext?, args: Array<Object?>?): Object? {
        if (args!!.size < 2 || args.size > 3) throw FunctionException(pc, "endsWith", 2, 3, args.size)
    ...
    companion object {
        fun call(pc: PageContext?, input: String?, subs: String?, ignoreCase: Boolean): Boolean {
            return if (ignoreCase) input.regionMatches(true, input!!.length() - subs!!.length(), subs, 0, subs!!.length()) else input.endsWith(subs)
        }
