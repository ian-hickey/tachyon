...
object ConfigFactory {
    ...
    fun getNew(engine: CFMLEngine?, contextDir: Resource?, readOnly: Boolean): UpdateInfo? {
        val info: lucee.Info = engine.getInfo()
    ...
    fun loadDocumentCreateIfFails(configFile: Resource?, type: String?): Struct? {
        return try {
            ...
            if (configFile.exists()) {
                ...
                while (configDir.getRealResource("lucee-" + type + "." + count++ + ".buggy").also { bugFile = it }.exists()) {
                }
    ...
    fun translateConfigFile(config: ConfigPro?, configFileOld: Resource?, configFileNew: Resource?, mode: String?, isServer: Boolean) {
        ...
        var root: Struct? = ConfigWebUtil.getAsStruct("cfLuceeConfiguration", XMLConfigReader(configFileOld, true, ReadRule(), NameRule()).getData())
    ...
    private fun _loadDocument(res: Resource?): Struct? {
        ...
        return if (StringUtil.endsWithIgnoreCase(name, ".xml.cfm") || StringUtil.endsWithIgnoreCase(name, ".xml")) {
            try {
                ConfigWebUtil.getAsStruct("cfLuceeConfiguration", XMLConfigReader(res, true, ReadRule(), NameRule()).getData())
            ...
            val ls: Resource = dir.getRealResource("lucee-server.xml")
            val lw: Resource = dir.getRealResource("lucee-web.xml.cfm")
