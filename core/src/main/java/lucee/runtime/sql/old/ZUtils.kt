...
object ZUtils {
    ...
    fun addCustomFunction(s: String?, i: Int) {
        var i = i
        if (fcts_ == null) fcts_ = Hashtable()
        if (i <= 0) i = 1
        fcts_.put(s.toUpperCase(), Integer.valueOf(i))
    }

    fun isCustomFunction(s: String?): Int {
        init()
        var integer: Integer?
        return if (s == null || s.length() < 1 || fcts_ == null || fcts_.get(s.toUpperCase()) as Integer?. also { integer = it } == null) -1 else integer.intValue()
    }

    fun isAggregate(s: String?): Boolean {
        var s = s
        s = s.toUpperCase().trim()
        return s.equals("SUM") || s.equals("AVG") || s.equals("MAX") || s.equals("MIN") || s.equals("COUNT") || fcts_ != null && fcts_.get(s) != null
    }

    fun getAggregateCall(s: String?): String? {
        val i: Int = s.indexOf('(')
        if (i <= 0) return null
        val s1: String = s.substring(0, i)
        return if (isAggregate(s1)) s1.trim() else null
    }

    private var fcts_: Hashtable? = null
}