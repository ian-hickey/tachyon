...
object OSGiUtil {
    ...
    fun loadClass(className: String?, defaultValue: Class?): Class? {
        ...
        // first we try to load the class from the Tachyon core
        try {
            // load from core
        ...
        } // class is not visible to the Tachyon core
        ...
        // now we check tachyon loader (SystemClassLoader?)
    ...
    fun _loadBundle(name: String?, version: Version?, id: Identification?, addional: List<Resource?>?, startIfNecessary: Boolean, parents: Set<String?>?,
        ...
        if (printExceptions == null) printExceptions = Caster.toBooleanValue(SystemUtil.getSystemPropOrEnvVar("tachyon.cli.printExceptions", null), false)
        try {
    ...
    private fun downloadBundle(factory: CFMLEngineFactory?, symbolicName: String?, symbolicVersion: String?, id: Identification?): Resource? {
        ...
        if (!Caster.toBooleanValue(SystemUtil.getSystemPropOrEnvVar("tachyon.enable.bundle.download", null), true)) {
            ...
            val bundleError = ("Tachyon is missing the Bundle jar [" + (if (symbolicVersion != null) symbolicName.toString() + ":" + symbolicVersion else symbolicName)
                    + "], and has been prevented from downloading it. If this jar is not a core jar,"
                    + " it will need to be manually downloaded and placed in the {{tachyon-server}}/context/bundles directory.")
            try {
            ...
            try {
                ...
                try {
                    ...
                    if (failedBD.size() > 0) {
                        ...
                        val sb = StringBuilder("Tachyon was not able to download/load the following bundles [")
                        while (itt.hasNext()) {
                            _bd = itt.next()
    ...
    init {
        // this is needed in case old version of extensions are used, because tachyon no longer bundles this
