...
object GetProfileString : Function {
    @Throws(PageException::class)
    fun call(pc: PageContext?, fileName: String?, section: String?, key: String?): String? {
        return try {
            val res: Resource = ResourceUtil.toResourceNotExisting(pc, fileName)
            if (!res.isFile()) return ""
            val ini = IniFile(res)
            val str: String = ini.getKeyValueEL(section, key) ?: return ""
            str
        } catch (e: IOException) {
