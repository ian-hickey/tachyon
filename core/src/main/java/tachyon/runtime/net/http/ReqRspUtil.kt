...
object ReqRspUtil {
        ...
        if (values != null) {
            ...
            try {
                ...
                while (values.hasMoreElements()) {
                    ...
                    val arr: Array<String?> = tachyon.runtime.type.util.ListUtil.listToStringArray(`val`, ';')
                    var tmp: Array<String?>
                    var c: Cookie?
                    for (i in arr.indices) {
                        tmp = tachyon.runtime.type.util.ListUtil.listToStringArray(arr[i], '=')
                        if (tmp.size > 0) {
     ...
     *
     * @param pc
     * @param deserialized if true tachyon tries to deserialize the body based on the content-type, for
