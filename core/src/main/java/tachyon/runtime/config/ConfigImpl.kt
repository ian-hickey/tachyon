...
/**
 * Hold the definitions of the Tachyon configuration.
 */
abstract class ConfigImpl protected constructor(configDir: Resource?, configFile: Resource?) : ConfigBase(), ConfigPro {
    ...
    private var tachyonTlds: Array<TagLib?>? = arrayOfNulls<TagLib?>(0)
    private var cfmlFlds: Array<FunctionLib?>? = arrayOfNulls<FunctionLib?>(0)
    private var tachyonFlds: Array<FunctionLib?>? = arrayOfNulls<FunctionLib?>(0)
    private var combinedCFMLFLDs: FunctionLib? = null
    private var combinedTachyonFLDs: FunctionLib? = null
    ...
    private var baseComponentPageSourceTachyon: PageSource? = null
    private var baseComponentTemplateTachyon: String? = null
    ...
    private var restMappings: Array<tachyon.runtime.rest.Mapping?>?
    ...
    private var regex // TODO add possibility to configure
            : Regex? = null
    private var applicationPathCacheTimeout: Long = Caster.toLongValue(SystemUtil.getSystemPropOrEnvVar("tachyon.application.path.cache.timeout", null), 20000)
    ...
    fun setFLDs(flds: Array<FunctionLib?>?, dialect: Int) {
        if (dialect == CFMLEngine.DIALECT_CFML) {
            ...
            tachyonFlds = flds
            combinedTachyonFLDs = null // TODO improve check (hash)
        }
    }
    ...
    override fun getFLDs(dialect: Int): Array<FunctionLib?>? {
        return if (dialect == CFMLEngine.DIALECT_CFML) cfmlFlds else tachyonFlds
    }
    ...
    override fun getCombinedFLDs(dialect: Int): FunctionLib? {
        ...
        if (combinedTachyonFLDs == null) combinedTachyonFLDs = FunctionLibFactory.combineFLDs(tachyonFlds)
        return combinedTachyonFLDs
    }
    ...
    override fun getTLDs(dialect: Int): Array<TagLib?>? {
        return if (dialect == CFMLEngine.DIALECT_CFML) cfmlTlds else tachyonTlds
    }

    fun setTLDs(tlds: Array<TagLib?>?, dialect: Int) {
        if (dialect == CFMLEngine.DIALECT_CFML) cfmlTlds = tlds else tachyonTlds = tlds
    }
    ...
    override fun getRestMappings(): Array<tachyon.runtime.rest.Mapping?>? {
        if (restMappings == null) restMappings = arrayOfNulls<tachyon.runtime.rest.Mapping?>(0)
        return restMappings
    }

    fun setRestMappings(restMappings: Array<tachyon.runtime.rest.Mapping?>?) {
    ...
    /**
     * set how tachyon cascade scopes
     *
    ...
    fun addTag(nameSpace: String?, nameSpaceSeperator: String?, name: String?, dialect: Int, cd: ClassDefinition?) {
        ...
        val tlds: Array<TagLib?>? = if (dialect == CFMLEngine.DIALECT_CFML) cfmlTlds else tachyonTlds
    ...
    fun setTldFile(fileTld: Resource?, dialect: Int) {
        ...
        var tlds: Array<TagLib?>? = if (dialect == CFMLEngine.DIALECT_CFML) cfmlTlds else tachyonTlds
    ...
    fun createTag(tl: TagLib?, filename: String?, mappingName: String?) { // Jira 1298
        ...
        tlt.setName(name)
        tlt.setTagClassDefinition("tachyon.runtime.tag.CFTagCore", getIdentification(), null)
    ...
    fun setFunctionDirectory(listFunctionDirectory: List<Path?>?) {
        ...
        while (it.hasNext()) {
            ...
            val fll: FunctionLib? = tachyonFlds!![tachyonFlds!!.size - 1]
    ...
    fun createFunction(fl: FunctionLib?, filename: String?, mapping: String?) {
        ...
        flf.setFunctionClass("tachyon.runtime.functions.system.CFFunction", null, null)
        flf.setName(name)
    ...
    fun setFldFile(fileFld: Resource?, dialect: Int) {
        ...
        var flds: Array<FunctionLib?>? = if (dialect == CFMLEngine.DIALECT_CFML) cfmlFlds else tachyonFlds
        ...
        if (dialect == CFMLEngine.DIALECT_CFML) {
            ...
            tachyonFlds = flds
            if (tachyonFlds != flds) combinedTachyonFLDs = null // TODO improve check
        }
    ...
    /**
     * set if tachyon make debug output or not
     *
    ...
    fun getBaseComponentTemplate(dialect: Int): String? {
        return if (dialect == CFMLEngine.DIALECT_CFML) baseComponentTemplateCFML else baseComponentTemplateTachyon
    }
    ...
    override fun getBaseComponentPageSource(dialect: Int, pc: PageContext?): PageSource? {
        var base: PageSource = if (dialect == CFMLEngine.DIALECT_CFML) baseComponentPageSourceCFML else baseComponentPageSourceTachyon
        if (base == null) {
            ...
            if (dialect == CFMLEngine.DIALECT_CFML) baseComponentPageSourceCFML = base else baseComponentPageSourceTachyon = base
        }
        return base
    }
    ...
    fun setBaseComponentTemplate(dialect: Int, template: String?) {
        if (dialect == CFMLEngine.DIALECT_CFML) {
            ...
            baseComponentPageSourceTachyon = null
            baseComponentTemplateTachyon = template
        }
    }
    ...
    /**
     * set the deploy directory, directory where tachyon deploy transalted cfml classes (java and class
     * files)
     *
    ...
    fun setComponentDefaultImport(str: String?) {
        ...
        if ("org.railo.cfml.*".equalsIgnoreCase(str)) str = "org.tachyon.cfml.*"
    ...
    private var allowTachyonDialect = false

    @Override
    override fun allowTachyonDialect(): Boolean {
        return allowTachyonDialect
    }

    fun setAllowTachyonDialect(allowTachyonDialect: Boolean) {
        this.allowTachyonDialect = allowTachyonDialect
    }
