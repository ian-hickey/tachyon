 ...
 *
 */
package tachyon.runtime.op.validators
...
object ValidateCreditCard {
     ...
     */
    const val DINERS = 2 // includes Carte Blanche
     ...
     * enum for Mastercard
     */
    const val MASTERCARD = 6

    /**
     * enum for insufficient digits
     */
    const val NOT_ENOUGH_DIGITS = -3

    /**
     * enum for too many digits
     */
    const val TOO_MANY_DIGITS = -2

    /**
     * enum for unknown vendor
     */
    const val UNKNOWN_VENDOR = -1
     ...
     */
    private var cachedLastFind = 0

    /**
     * ranges of credit card number that belong to each company. buildRanges initialises.
     */
    private var ranges: Array<LCR>

    /**
     * used by vendorToString to describe the enumerations
     */
    private val vendors = arrayOf("Error: not enough digits", "Error: too many digits", "Error: unknown credit card company", "dummy", "Amex", "Diners/Carte Blanche",
    ...
    private fun buildRanges() {
        ...
        ranges = arrayOf(LCR(4000000000000L, 4999999999999L, 13, VISA, true), LCR(30000000000000L, 30599999999999L, 14, DINERS, true),
                LCR(36000000000000L, 36999999999999L, 14, DINERS, true), LCR(38000000000000L, 38999999999999L, 14, DINERS, true),
                LCR(180000000000000L, 180099999999999L, 15, JCB, true), LCR(201400000000000L, 201499999999999L, 15, ENROUTE, false),
                LCR(213100000000000L, 213199999999999L, 15, JCB, true), LCR(214900000000000L, 214999999999999L, 15, ENROUTE, false),
                LCR(340000000000000L, 349999999999999L, 15, AMEX, true), LCR(370000000000000L, 379999999999999L, 15, AMEX, true),
                LCR(3000000000000000L, 3999999999999999L, 16, JCB, true), LCR(4000000000000000L, 4999999999999999L, 16, VISA, true),
                LCR(5100000000000000L, 5599999999999999L, 16, MASTERCARD, true), LCR(6011000000000000L, 6011999999999999L, 16, DISCOVER, true)) // end table
        // initialisation
    }
     ...
     */
    internal fun findMatchingRange(creditCardNumber: Long): Int {
    ...
    fun isValid(strCreditCardNumber: String?): Boolean {
        return isValid(toLong(strCreditCardNumber, 0L))
    }

    private fun toLong(strCreditCardNumber: String?, defaultValue: Long): Long {
        if (strCreditCardNumber == null) return defaultValue

        // strip commas, spaces, + AND -
        val sb = StringBuffer(strCreditCardNumber.length())
        for (i in 0 until strCreditCardNumber.length()) {
            val c: Char = strCreditCardNumber.charAt(i)
            if (!StringUtil.isWhiteSpace(c) && c != ',' && c != '+' && c != '-') sb.append(c)
        }
        val num = Caster.toDoubleValue(sb.toString(), 0L) as Long
        return if (num == 0L) defaultValue else num
    }
     ...
     */
    fun isValid(creditCardNumber: Long): Boolean {
        val i = findMatchingRange(creditCardNumber)
        if (i < 0) {
            return false
        }
        // else {
     ...
     */
    fun recognizeVendor(creditCardNumber: Long): Int {
        val i = findMatchingRange(creditCardNumber)
        return if (i < 0) {
            i
        } else ranges[i].vendor
    } // end recognize
    ...
    fun toCreditcard(strCreditCardNumber: String): String {
        val number = toLong(strCreditCardNumber, -1)
        if (number == -1L) throw ExpressionException("invalid creditcard number [$strCreditCardNumber]")
        return toPrettyString(number)
    }

    fun toCreditcard(strCreditCardNumber: String?, defaultValue: String): String {
        val number = toLong(strCreditCardNumber, -1)
        return if (number == -1L) defaultValue else toPrettyString(number)
    }
     ...
     */
    private fun toPrettyString(creditCardNumber: Long): String {
        val plain = toString(creditCardNumber)
        // int i = findMatchingRange(creditCardNumber);
        val length: Int = plain.length()
        return when (length) {
            12 ->            // 12 pattern 3-3-3-3
                plain.substring(0, 3) + ' ' + plain.substring(3, 6) + ' ' + plain.substring(6, 9) + ' ' + plain.substring(9, 12)
            13 ->            // 13 pattern 4-3-3-3
                plain.substring(0, 4) + ' ' + plain.substring(4, 7) + ' ' + plain.substring(7, 10) + ' ' + plain.substring(10, 13)
            14 ->            // 14 pattern 2-4-4-4
                plain.substring(0, 2) + ' ' + plain.substring(2, 6) + ' ' + plain.substring(6, 10) + ' ' + plain.substring(10, 14)
            15 ->            // 15 pattern 3-4-4-4
                plain.substring(0, 3) + ' ' + plain.substring(3, 7) + ' ' + plain.substring(7, 11) + ' ' + plain.substring(11, 15)
            16 ->            // 16 pattern 4-4-4-4
                plain.substring(0, 4) + ' ' + plain.substring(4, 8) + ' ' + plain.substring(8, 12) + ' ' + plain.substring(12, 16)
            17 ->            // 17 pattern 1-4-4-4-4
                plain.substring(0, 1) + ' ' + plain.substring(1, 5) + ' ' + plain.substring(5, 9) + ' ' + plain.substring(9, 13) + ' ' + plain.substring(13, 17)
            else ->            // 0..11, 18+ digits long
                // plain
                plain
        }
    } // end toPrettyString
     ...
     */
    fun vendorToString(vendorEnum: Int): String {
