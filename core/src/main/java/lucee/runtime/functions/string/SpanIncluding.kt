...
class SpanIncluding : BIF() {
    @Override
    @Throws(PageException::class)
    operator fun invoke(pc: PageContext?, args: Array<Object?>?): Object? {
        if (args!!.size == 2) return call(pc, Caster.toString(args[0]), Caster.toString(args[1]))
        throw FunctionException(pc, "SpanIncluding", 2, 2, args.size)
    }

    companion object {
        private const val serialVersionUID = 3274154577208880461L
        fun call(pc: PageContext?, str: String?, set: String?): String? {
            ...
            if (stringtokenizer.hasMoreTokens()) {
                ...
                return if (i == 0) "" else str.substring(0, i)
            }
            return str
        }
    }
