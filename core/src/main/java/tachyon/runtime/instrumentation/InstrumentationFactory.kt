...
object InstrumentationFactory {
    ...
    private val AGENT_CLASS_NAME: String? = "tachyon.runtime.instrumentation.ExternalAgent"
    ...
    private fun _getInstrumentation(cl: ClassLoader?, log: Log?): Instrumentation? {
        ...
        if (clazz != null) {
            log.info("Instrumentation", "found [tachyon.runtime.instrumentation.ExternalAgent] in ClassLoader [" + clazz.getClassLoader().toString() + "]")
        } else {
            log.info("Instrumentation", "not found [tachyon.runtime.instrumentation.ExternalAgent] in ClassLoader [$cl]")
            return null
        }
        try {
    ...
    private fun createAgentJar(log: Log?, c: Config?): Resource? {
        val trg: Resource = getDeployDirectory(c).getRealResource("tachyon-external-agent.jar")
        if (!trg.exists() || trg.length() === 0) {
            log.info("Instrumentation", "create $trg")
            val jar: InputStream = InfoImpl::class.java.getResourceAsStream("/resource/lib/tachyon-external-agent.jar")
                    ?: throw IOException("could not load jar [/resource/lib/tachyon-external-agent.jar]")
