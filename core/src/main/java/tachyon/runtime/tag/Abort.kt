...
class Abort : TagImpl() {
    ...
    private var type: Int = tachyon.runtime.exp.Abort.SCOPE_REQUEST
    ...
    fun setType(type: String?) {
        ...
        if (type.equals("page")) this.type = tachyon.runtime.exp.Abort.SCOPE_PAGE else if (type.equals("request")) this.type = tachyon.runtime.exp.Abort.SCOPE_REQUEST else throw ApplicationException("attribute type has an invalid value [$type], valid values are [page,request]")
    }
    ...
    fun release() {
        super.release()
        showerror = null
        type = tachyon.runtime.exp.Abort.SCOPE_REQUEST
    }
}