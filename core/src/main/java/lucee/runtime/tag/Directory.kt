        ...
        fun actionList(pageContext: PageContext?, directory: Resource?, serverPassword: String?, type: Int, filter: ResourceFilter?, listInfo: Int, recurse: Boolean, sort: String?): Object? {
                ...
                for (i in arr.indices.reversed()) {
                    try {
                        ...
                        if (col.size == 1) query.sort(col[0].trim()) else if (col.size == 2) {
                            ...
                            if (order.equals("asc")) query.sort(col[0], lucee.runtime.type.Query.ORDER_ASC) else if (order.equals("desc")) query.sort(col[0], lucee.runtime.type.Query.ORDER_DESC) else throw ApplicationException("Invalid order type [" + col[1] + "]")
                        }
                    } catch (t: Throwable) {
            ...
            if (typeArray) {
                ...
                while (it.hasNext()) {
                    ...
                    if (namesOnly) array.appendEL(row.get("name")) else array.appendEL(row.get("directory") + lucee.commons.io.FileUtil.FILE_SEPERATOR_STRING + row.get("name"))
                }
            }
            return rtn
        }
