...
class ArrayEvery : BIF() {
    ...
    operator fun invoke(pc: PageContext?, args: Array<Object?>?): Object? {
        ...
        throw FunctionException(pc, "ArrayEvery", 2, 4, args.size)
    }

    companion object {
        private const val serialVersionUID = -7449844630816343951L
        @Throws(PageException::class)
        fun call(pc: PageContext?, array: Array?, udf: UDF?): Boolean {
            return _call(pc, array, udf, false, 20)
        }

        @Throws(PageException::class)
        fun call(pc: PageContext?, array: Array?, udf: UDF?, parallel: Boolean): Boolean {
            return _call(pc, array, udf, parallel, 20)
        }

        @Throws(PageException::class)
        fun call(pc: PageContext?, array: Array?, udf: UDF?, parallel: Boolean, maxThreads: Double): Boolean {
            return _call(pc, array, udf, parallel, maxThreads.toInt())
        }

        @Throws(PageException::class)
        private fun _call(pc: PageContext?, array: Array?, udf: UDF?, parallel: Boolean, maxThreads: Int): Boolean {
