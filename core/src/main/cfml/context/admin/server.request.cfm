...
<cfset stText.application.appPathTimeoutDesc="If set to greater than 0 Tachyon will cache the Path to the Application.[cfc|cfm] file to use for that time. So Tachyon does not search the Application.cfc with every request. If set to 0 the cache is disabled. ">
