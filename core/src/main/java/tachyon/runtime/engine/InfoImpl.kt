...
class InfoImpl @JvmOverloads constructor(bundle: Bundle? = null) : Info {
    ...
    fun getTachyonTemplateExtensions(): Array<String?>? {
        return Constants.getTachyonTemplateExtensions()
    }
    ...
    @Override
    fun getTachyonComponentExtensions(): Array<String?>? {
        return arrayOf(getTachyonComponentExtension())
    }
    ...
    fun getTachyonComponentExtension(): String? {
        return Constants.getTachyonComponentExtension()
    }
    ...
    companion object {
        ...
        private fun load(`is`: InputStream?): Manifest? {
            try {
                ...
                if ("tachyon.core".equals(sn)) return m
            } catch (t: Throwable) {
                ExceptionUtil.rethrowIfNecessary(t)
            } finally {
                IOUtil.closeEL(`is`)
