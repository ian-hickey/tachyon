...
class FDControllerImpl(engine: CFMLEngineImpl?, serial: String?) : IFDController {
    ...
    fun getLicenseInformation(key: String?): String? {
        if (!isEnterprise) {
            LogUtil.log(Log.LEVEL_ERROR, "integral", "FD Server Licensing does not work with the Open Source Version of Tachyon or Enterprise Version of Tachyon that is not enabled")
            return null
        }
