...
class ClassicHTMLDumpWriter : DumpWriter {
    @Override
    ...
    private fun writeOut(pc: PageContext?, data: DumpData?, writer: Writer?, expand: Boolean, inside: Boolean) {
        ...
        var borderColor: String = table.getHighLightColor()
        val h1Color: String = table.getHighLightColor()
        val h2Color: String = table.getNormalColor()
        val normalColor = "white"
        try {
            val color: Color = ColorCaster.toColor(table.getHighLightColor())
            borderColor = ColorCaster.toHexString(color.darker().darker())
        } catch (e: ExpressionException) {
        }
        var id: String? = "_classic" + count++
        // prepare data
        ...
        for (i in rows.indices) {
            ...
            for (y in 0 until cols) {
                ...
                writer.write(" bgcolor=\"" + (if (highLightIt) h2Color else normalColor) + "\" style=\"border : 1px solid " + borderColor + ";empty-cells:show;\">")
