...
class DebuggerImpl : Debugger {
    ...
    fun writeOut(pc: PageContext?) {
        // stop();
        if (!output) return
        val debugEntry: tachyon.runtime.config.DebugEntry = getDebugEntry(pc)
            ...
            if (p == null) {
                if (path != null) {
                    ...
                    if (path.endsWith("/Modern.cfc") || path.endsWith("\\Modern.cfc")) {
                        path = "/tachyon-server-context/admin/debug/Modern.cfc"
                        fullname = "tachyon-server-context.admin.debug.Modern"
                        changed = true
                    } else if (path.endsWith("/Classic.cfc") || path.endsWith("\\Classic.cfc")) {
                        path = "/tachyon-server-context/admin/debug/Classic.cfc"
                        fullname = "tachyon-server-context.admin.debug.Classic"
                        changed = true
                    } else if (path.endsWith("/Comment.cfc") || path.endsWith("\\Comment.cfc")) {
                        path = "/tachyon-server-context/admin/debug/Comment.cfc"
                        fullname = "tachyon-server-context.admin.debug.Comment"
                        changed = true
                    }
                    if (changed) pc.write(
                            "<span style='color:red'>Please update your debug template definitions in the Tachyon admin by going into the detail view and hit the \"update\" button.</span>")
                }
    ...
    companion object {
         ...
         * @return
         */
        fun getDebugEntry(pc: PageContext?): tachyon.runtime.config.DebugEntry? {
