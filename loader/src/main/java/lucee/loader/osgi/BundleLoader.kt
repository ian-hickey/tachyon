...
object BundleLoader {
    ...
    fun loadBundles(engFac: CFMLEngineFactory, cacheRootDir: File?, jarDirectory: File, rc: File, old: BundleCollection?): BundleCollection {
        ...
        return try {
            ...
            val mani: Manifest = jf.getManifest()
                    ?: throw IOException("lucee core [$rc] is invalid, there is no META-INF/MANIFEST.MF File")
            ...
            if (Util.isEmpty(rb)) throw IOException("lucee core [$rc] is invalid, no Require-Bundle definition found in the META-INF/MANIFEST.MF File")
    ...
    fun loadBundleInfo(jar: File): String {
        ...
        return try {
            ...
            if (Util.isEmpty(symbolicName)) throw IOException("OSGi bundle [$jar] is invalid, {Lucee-Core}META-INF/MANIFEST.MF does not contain a \"Bundle-SymbolicName\"")
            if (Util.isEmpty(version)) throw IOException("OSGi bundle [$jar] is invalid, {Lucee-Core}META-INF/MANIFEST.MF does not contain a \"Bundle-Version\"")
    ...
    fun loadDefaultProperties(jf: JarFile): Properties {
        val ze: ZipEntry = jf.getEntry("default.properties")
                ?: throw IOException("the Lucee core has no default.properties file!")
        val prop = Properties()
