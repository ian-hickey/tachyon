...
object MemberUtil {
    ...
    private val matchesLucee: Map<Short?, Map<Collection.Key?, FunctionLibFunction?>?>? = HashMap<Short?, Map<Collection.Key?, FunctionLibFunction?>?>()
    ...
    fun getMembers(pc: PageContext?, type: Short): Map<Collection.Key?, FunctionLibFunction?>? {
        val matches: Map<Short?, Map<Key?, FunctionLibFunction?>?>? = if (pc.getCurrentTemplateDialect() === CFMLEngine.DIALECT_LUCEE) matchesLucee else matchesCFML
