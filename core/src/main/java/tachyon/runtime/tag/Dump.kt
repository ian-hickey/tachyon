...
class Dump : TagImpl() {
    ...
    fun doStartTag(): Int {
        ...
        tachyon.runtime.functions.other.Dump.call(pageContext, `var`, label, expand, top.toDouble(), show, hide, output, format, keys, metainfo, showUDFs)
        if (abort) throw Abort(tachyon.runtime.exp.Abort.SCOPE_REQUEST)
        return SKIP_BODY
    }
