...
abstract class ComponentPageImpl : ComponentPage(), PagePro {
    ...
    fun call(pc: PageContext?): Object? {
        ...
        val internalCall = "lucee-gateway-1-0".equals(client) || "lucee-listener-1-0".equals(client)
        val fromRest = "lucee-rest-1-0".equals(client)
        ...
        try {
            ...
            if (path.size() > 1 && !(path.size() === 3 && ListUtil.last(path.getE(2).toString(), "/\\", true)
                            .equalsIgnoreCase(if (pc.getRequestDialect() === CFMLEngine.DIALECT_CFML) lucee.runtime.config.Constants.CFML_APPLICATION_EVENT_HANDLER else lucee.runtime.config.Constants.LUCEE_APPLICATION_EVENT_HANDLER))) { // MUSTMUST
                // bad
                // impl
                // ->
                // check
            ...
            // DUMP
            if (!req.getServletPath().equalsIgnoreCase("/Web." + if (pc.getRequestDialect() === CFMLEngine.DIALECT_CFML) lucee.runtime.config.Constants.getCFMLComponentExtension() else lucee.runtime.config.Constants.getLuceeComponentExtension())) {
    ...
    companion object {
        ...
        val REMOTE_PERSISTENT_ID: lucee.runtime.type.Collection.Key? = KeyImpl.getInstance("Id16hohohh")
