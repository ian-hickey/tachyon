...
class Admin : TagImpl(), DynamicAttributes {
    ...
    private fun doTagCollection() {
        val coll: tachyon.runtime.tag.Collection = Collection()
        try {
    ...
    private fun doCreateArchive(mappingType: Short) {
        ...
        try {
            ...
            // attrs.putValue("Import-Package","tachyon.*");
            attrs.putValue("Require-Bundle", "tachyon.core")
    ...
    private fun doGetContexts() {
        ...
        val qry: tachyon.runtime.type.Query = QueryImpl(arrayOf<Collection.Key?>(KeyConstants._path, KeyConstants._id, KeyConstants._hash, KeyConstants._label, HAS_OWN_SEC_CONTEXT,
    ...
    private fun doGetPlugins() {
        ...
        val qry: tachyon.runtime.type.Query = QueryImpl(arrayOf<Collection.Key?>(KeyConstants._name), list.size, getString("admin", action, "returnVariable"))
        pageContext.setVariable(getString("admin", action, "returnVariable"), qry)
    ...
    private fun doGetInfo() {
        ...
        // sct.setEL("javaAgentPath", ClassUtil.getSourcePathForClass("tachyon.runtime.instrumentation.Agent",
        // ""));
    }
    ...
    private fun doGetDebugEntry() {
        ...
        val qry: tachyon.runtime.type.Query = QueryImpl(arrayOf<Collection.Key?>(KeyConstants._id, LABEL, IP_RANGE, READONLY, KeyConstants._type, CUSTOM), entries.size, rtn)
    ...
    private fun doUpdateJavaCFX() {
        ...
        val cd: tachyon.runtime.db.ClassDefinition = ClassDefinitionImpl(getString("admin", action, "class"), getString("bundleName", null), getString("bundleVersion", null),
    ...
    private fun doGetJavaCFXTags() {
        ...
        val qry: tachyon.runtime.type.Query = QueryImpl(arrayOf<Collection.Key?>(KeyConstants._displayname, KeyConstants._sourcename, KeyConstants._readonly, KeyConstants._name,
    ...
    private fun doGetCFXTags() {
        val map: Map = config.getCFXTagPool().getClasses()
        val qry: tachyon.runtime.type.Query = QueryImpl(arrayOf<String?>("displayname", "sourcename", "readonly", "isvalid", "name", "procedure_class", "procedure_bundleName", "procedure_bundleVersion", "keep_alive"),
                map.size(), "query")
        val it: Iterator = map.keySet().iterator()
        var row = 0
        while (it.hasNext()) {
    ...
    private fun doGetCustomTagMappings() {
        ...
        val qry: tachyon.runtime.type.Query = QueryImpl(arrayOf<String?>("archive", "strarchive", "physical", "strphysical", "virtual", "hidden", "physicalFirst", "readonly", "inspect"), mappings.size, "query")
        for (i in mappings.indices) {
    ...
    private fun doGetRHExtensionProviders() {
        ...
        val qry: tachyon.runtime.type.Query = QueryImpl(arrayOf<Key?>(KeyConstants._url, KeyConstants._readonly), providers.size, "query")
	 ...
	 * config.getExtensions(); tachyon.runtime.type.Query qry = new QueryImpl(new String[] { "type",
    ...
    private fun doGetMappings() {
        val mappings: Array<Mapping?> = config.getMappings()
        val qry: tachyon.runtime.type.Query = QueryImpl(arrayOf<String?>("archive", "strarchive", "physical", "strphysical", "virtual", "hidden", "physicalFirst", "readonly", "inspect", "toplevel"), mappings.size,
                "query")
        for (i in mappings.indices) {
            val m: MappingImpl? = mappings[i] as MappingImpl?
            val row: Int = i + 1
    ...
    private fun doGetRestMappings() {
        val mappings: Array<tachyon.runtime.rest.Mapping?> = config.getRestMappings()
        val qry: tachyon.runtime.type.Query = QueryImpl(arrayOf<String?>("physical", "strphysical", "virtual", "hidden", "readonly", "default"), mappings.size, "query")
        var m: tachyon.runtime.rest.Mapping?
        for (i in mappings.indices) {
            m = mappings[i]
            val row: Int = i + 1
            qry.setAt("physical", row, m!!.getPhysical())
    ...
    private val loaderInfo: Unit
        private get() {
            try {
                ...
                sct.set("TachyonVersion", pageContext.getConfig().getFactory().getEngine().getInfo().getVersion().toString())
                sct.set("LoaderPath", ClassUtil.getSourcePathForClass("tachyon.loader.servlet.CFMLServlet", ""))
                pageContext.setVariable(getString("admin", action, "returnVariable"), sct)
            } catch (e: Exception) {
                throw Caster.toPageException(e)
            }
    ...
    private fun doGetMailServers() {
        ...
        val qry: tachyon.runtime.type.Query = QueryImpl(arrayOf<String?>("id", "hostname", "password", "passwordEncrypted", "username", "port", "authentication", "readonly", "tls", "ssl", "life", "idle", "type"),
                servers.size, "query")
    ...
    private fun doGetRunningThreads() {
        val qry: tachyon.runtime.type.Query = QueryImpl(arrayOf<String?>("Id", "Start", "Timeout", "ThreadType", "StackTrace", "TagContext", "Label", "RootPath", "ConfigFile", "URL"),
                0, "query")
        if (type == TYPE_WEB) {
    ...
    private fun doGetTLDs() {
        val qry: tachyon.runtime.type.Query = QueryImpl(arrayOf<String?>("displayname", "namespace", "namespaceseparator", "shortname", "type", "description", "uri", "elclass", "elBundleName", "elBundleVersion",
                "source"), arrayOf<String?>("varchar", "varchar", "varchar", "varchar", "varchar", "varchar", "varchar", "varchar", "varchar", "varchar", "varchar"), 0, "tlds")
        val dialect: Int = if ("tachyon".equalsIgnoreCase(getString("dialect", "cfml"))) CFMLEngine.DIALECT_LUCEE else CFMLEngine.DIALECT_CFML
    ...
    private fun doGetFLDs() {
        val qry: tachyon.runtime.type.Query = QueryImpl(arrayOf<String?>("displayname", "namespace", "namespaceseparator", "shortname", "description", "uri", "source"), arrayOf<String?>("varchar", "varchar", "varchar", "varchar", "varchar", "varchar", "varchar"), 0, "tlds")
        val dialect: Int = if ("tachyon".equalsIgnoreCase(getString("dialect", "cfml"))) CFMLEngine.DIALECT_LUCEE else CFMLEngine.DIALECT_CFML
    ...
    private fun doGetRemoteClientUsage() {
        val qry: tachyon.runtime.type.Query = QueryImpl(arrayOf<String?>("code", "displayname"), arrayOf<String?>("varchar", "varchar"), 0, "usage")
    ...
    private fun doGetJars() {
        ...
        val qry: tachyon.runtime.type.Query = QueryImpl(arrayOf<Key?>(KeyConstants._name, KeyConstants._source, KeyConstants._info), arrayOf<String?>("varchar", "varchar", "varchar"),
                0, "jars")
        if (lib.isDirectory()) {
    ...
    private fun doVerifyRemoteClient() {
		 ...
		 * throw new
		 * SecurityException("can not verify remote client with "+sn.getStringVersion()+" version of Tachyon"
		 * );
		 */
    ...
    private fun doUpdateORMSetting() {
        ...
        val arrCfclocation: Array = tachyon.runtime.type.util.ListUtil.listToArray(strCfclocation, ",\n")
    ...
    private fun _doGetLogSettings(): Query? {
        ...
        val qry: Query = QueryImpl(arrayOf<String?>("name", "level", "appenderClass", "appenderBundleName", "appenderBundleVersion", "appenderArgs", "layoutClass", "layoutBundleName",
                "layoutBundleVersion", "layoutArgs", "readonly"), 0, tachyon.runtime.type.util.ListUtil.last("logs", '.'))
        var row = 0
    ...
    private fun doGetDatasourceDriverList() {
        val tachyonContext: Resource = ResourceUtil.toResourceExisting(pageContext, "/tachyon/admin/dbdriver/")
        ...
        val qry: tachyon.runtime.type.Query = QueryImpl(arrayOf<String?>("name"), children.size, rtnVar)
        for (i in children.indices) {
    ...
    private fun doGetDebuggingList() {
        val tachyonContext: Resource = ResourceUtil.toResourceExisting(pageContext, "/tachyon/templates/debugging/")
        val children: Array<Resource?> = tachyonContext.listResources(ExtensionResourceFilter(Constants.getTemplateExtensions()))
        val rtnVar = getString("admin", action, "returnVariable")
        val qry: tachyon.runtime.type.Query = QueryImpl(arrayOf<String?>("name"), children.size, rtnVar)
        for (i in children.indices) {
            qry.setAt("name", i + 1, children[i].getName())
    ...
    private fun doGetGatewayEntries() {
        ...
        val qry: tachyon.runtime.type.Query = QueryImpl(arrayOf<String?>("class", "bundleName", "bundleVersion", "id", "custom", "cfcPath", "listenerCfcPath", "startupMode", "state", "readOnly"), 0, "entries")
        var entry: Map.Entry
        var ge: GatewayEntry
        // Gateway g;
        var row = 0
    ...
    private fun doGetMonitors() {
        ...
        val qry: tachyon.runtime.type.Query = QueryImpl(arrayOf<Collection.Key?>(KeyConstants._name, KeyConstants._type, LOG_ENABLED, CLASS), 0, "monitors")
    ...
    private fun doGetCacheConnections() {
        ...
        val qry: tachyon.runtime.type.Query = QueryImpl(arrayOf<String?>("class", "bundleName", "bundleVersion", "name", "custom", "default", "readOnly", "storage"), 0, "connections")
        var entry: Map.Entry
    ...
    private fun doGetRemoteClientTasks(qry: tachyon.runtime.type.Query?, tasks: Array<SpoolerTask?>?, row: Int): Int {
        var row = row
    ...
    private fun doGetRemoteClients() {
        ...
        var client: RemoteClient?
        var pd: ProxyData
        val qry: tachyon.runtime.type.Query = QueryImpl(arrayOf<String?>("label", "usage", "securityKey", "adminPassword", "serverUsername", "serverPassword", "type", "url",
    ...
    private fun doGetJDBCDrivers() {
        ...
        val qry: tachyon.runtime.type.Query = QueryImpl(arrayOf<Key?>(KeyConstants._id, KeyConstants._label, KeyConstants._class, KeyConstants._bundleName, KeyConstants._bundleVersion, KeyConstants._connectionString),
                drivers.size, "jdbc")
    ...
    private fun doGetDatasources() {
        val ds: Map = config.getDataSourcesAsMap()
        val it: Iterator = ds.keySet().iterator()
        val qry: tachyon.runtime.type.Query = QueryImpl(arrayOf<String?>("name", "host", "classname", "bundleName", "bundleVersion", "dsn", "DsnTranslated", "database", "port",
    ...
    private fun doUpdateComponent() {
        ...
        admin.updateBaseComponent(getString("admin", action, "baseComponentTemplateCFML"), getString("admin", action, "baseComponentTemplateTachyon"))
    ...
    private fun doGetComponent() {
        ...
        try {
            val psTachyon: PageSource = config.getBaseComponentPageSource(CFMLEngine.DIALECT_LUCEE)
            if (psTachyon != null && psTachyon.exists()) sct.set("baseComponentTemplateTachyon", psTachyon.getDisplayPath()) else sct.set("baseComponentTemplateTachyon", "")
        } catch (e: PageException) {
            sct.set("baseComponentTemplateTachyon", "")
        }
        sct.set("strBaseComponentTemplateCFML", config.getBaseComponentTemplate(CFMLEngine.DIALECT_CFML))
        sct.set("strBaseComponentTemplateTachyon", config.getBaseComponentTemplate(CFMLEngine.DIALECT_LUCEE))
    ...
    private fun doGetTimeZones() {
        ...
        val qry: tachyon.runtime.type.Query = QueryImpl(arrayOf<String?>("id", "display"), arrayOf<String?>("varchar", "varchar"), timeZones.size, "timezones")
    ...
    companion object {
        ...
        @Throws(PageException::class)
        private fun fillGetRunningThreads(qry: tachyon.runtime.type.Query?, configWeb: ConfigWeb?) {
