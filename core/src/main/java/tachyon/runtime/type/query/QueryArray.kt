...
class QueryArray(private val name: String?, sql: SQL?, templateLine: TemplateLine?) : ArrayImpl(), QueryResult {
    private val sql: SQL?
    ...
    fun duplicate(deepCopy: Boolean): Collection? {
        val qa = QueryArray(name, sql, templateLine)
    ...
    companion object {
        ...
        @Throws(PageException::class)
        fun toQueryArray(q: tachyon.runtime.type.QueryImpl?): QueryArray? {
            val qa = QueryArray(q!!.getName(), q!!.getSql(), q!!.getTemplateLine())
            ...
            var tmp: Struct?
            for (r in 1..rows) {
                qa.add(StructImpl(Struct.TYPE_LINKED).also { tmp = it })
                for (c in columns) {
