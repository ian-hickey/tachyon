 ...
 * `http` liegen.
 */
class HttpParam : EvaluatorSupport() {
    @Override
    ...
    fun evaluate(tag: Tag?, libTag: TagLibTag?) {
        ...
        val httpName = ns + "http"

        // check if tag is direct inside if
        if (!ASMUtil.hasAncestorTag(tag, httpName)) throw EvaluatorException("Wrong Context, tag [" + libTag.getFullName().toString() + "] must be inside a [" + httpName + "] tag")
    }
}