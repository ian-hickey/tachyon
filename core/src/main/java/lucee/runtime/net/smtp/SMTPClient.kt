...
class SMTPClient : Serializable {
    ...
    private fun createMimeMessage(config: lucee.runtime.config.Config?, hostName: String?, port: Int, username: String?, password: String?, lifeTimesan: Long,
    ...
    @Throws(MessagingException::class)
    fun toMimeBodyPart(mp: Multipart?, config: lucee.runtime.config.Config?, att: Attachment?): MimeBodyPart? {
    ...
    fun _send(config: lucee.runtime.config.ConfigWeb?, servers: Array<Server?>?) {
    ...
    private val isUse7bitHtmlEncoding: Boolean
        private get() = try {
            Caster.toBoolean(SystemUtil.getSystemPropOrEnvVar("lucee.mail.use.7bit.transfer.encoding.for.html.parts", "false"))
        } catch (t: Throwable) {
            false
        }
