...
abstract class AbstrCFMLExprTransformer {
    ...
    private fun lambda(data: Data?): Expression? {
        ...
        if (!data.srcCode.forwardIfCurrent("(")) return null
        var args: ArrayList<tachyon.transformer.bytecode.statement.Argument?>? = null
        // data.cfml.previous();
        args = try {
    ...
    protected abstract fun lambdaPart(data: Data?, id: String?, access: Int, modifier: Int, rtnType: String?, line: Position?,
                                      args: ArrayList<tachyon.transformer.bytecode.statement.Argument?>?): Function?

    @Throws(TemplateException::class)
    protected abstract fun getScriptFunctionArguments(data: Data?): ArrayList<tachyon.transformer.bytecode.statement.Argument?>?
