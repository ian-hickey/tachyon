 ...
 * eqv operation
 */
class Xor(left: Ref?, right: Ref?, limited: Boolean) : RefSupport(), Ref {
    ...
    fun getValue(pc: PageContext?): Object? {
        ...
        return if (Caster.toBooleanValue(left.getValue(pc)) xor Caster.toBooleanValue(right.getValue(pc))) Boolean.TRUE else Boolean.FALSE
    }
