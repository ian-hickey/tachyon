...
class IKHandlerCache : IKHandler {
    protected var storeEmpty: Boolean = Caster.toBooleanValue(SystemUtil.getSystemPropOrEnvVar("lucee.store.empty", null), false)
    ...
    companion object {
        ...
        private fun getCache(pc: PageContext?, cacheName: String?): Cache? {
            return try {
                ...
                if (!cc.isStorage()) throw ApplicationException("storage usage for this cache is disabled, you can enable this in the Lucee administrator.")
        ...
        fun getKey(cfid: String?, appName: String?, type: String?): String? {
            return StringBuilder("lucee-storage:").append(type).append(":").append(cfid).append(":").append(appName).toString().toUpperCase()
        }
