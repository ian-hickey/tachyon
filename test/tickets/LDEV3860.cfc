component extends="org.tachyon.cfml.test.TachyonTestCase" labels="mssql,orm" {
    function beforeAll(){
    ...
    function run( testResults, testBox ) {
        ...
        describe(title="Testcase for LDEV-3860", body=function() {
            it( title="check error - using invalid entity name in entityNew() without transaction", skip="#isDatasourceNotConfigured()#", body=function( currentSpec ){
            ...
            it( title="check error - using invalid entity name in entityNew() inside transaction", skip="#isDatasourceNotConfigured()#", body=function( currentSpec ){
                local.res = _InternalRequest(
                    template="#variables.uri#\LDEV3860.cfm",
                    forms = {scene:3}
                );
