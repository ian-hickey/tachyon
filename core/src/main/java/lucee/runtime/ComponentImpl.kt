...
class ComponentImpl : StructSupport, Externalizable, Component, coldfusion.runtime.TemplateProxy {
    ...
    operator fun compareTo(b: Boolean): Int {
        return lucee.runtime.op.OpUtil.compare(ThreadLocalPageContext.get(), if (castToBooleanValue()) Boolean.TRUE else Boolean.FALSE, if (b) Boolean.TRUE else Boolean.FALSE)
    }

    @Override
    @Throws(PageException::class)
    operator fun compareTo(dt: DateTime?): Int {
        return lucee.runtime.op.OpUtil.compare(ThreadLocalPageContext.get(), castToDateTime() as Date?, dt as Date?)
    }
