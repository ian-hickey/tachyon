...
class BracketExpression(exp: Expression?) : ExpressionSupport() {
    private val exp: Expression?

    @Override
    override fun toString(noAlias: Boolean): String? {
        return if (noAlias || getIndex() === 0) "(" + exp!!.toString(true).toString() + ")" else toString(true).toString() + " as " + getIndex()
    }
