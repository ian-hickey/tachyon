...
class ConfigAdmin private constructor(config: ConfigPro?, password: Password?) {
    ...
    fun updateCacheConnection(name: String?, cd: ClassDefinition?, _default: Int, custom: Struct?, readOnly: Boolean, storage: Boolean) {
        ...
        try {
            val clazz: Class
            clazz = if (cd.getClassName() != null && cd.getClassName().endsWith(".EHCacheLite")) ClassUtil.loadClass(config.getClassLoader(), "org.tachyon.extension.cache.eh.EHCache") else ClassUtil.loadClass(config.getClassLoader(), cd.getClassName())
     ...
     */
    @Throws(SecurityException::class)
    fun updateBaseComponent(baseComponentCFML: String?, baseComponentTachyon: String?) {
        checkWriteAccess()
        val hasAccess: Boolean = ConfigWebUtil.hasAccess(config, SecurityManager.TYPE_SETTING)
        if (!hasAccess) throw SecurityException("no access to update component setting")
        root.setEL("componentBase", baseComponentCFML)
        root.setEL("componentBaseTachyonDialect", baseComponentTachyon)
    }
    ...
    /**
     * updates update settingd for Tachyon
     *
     * @param type
     * @param location
     * @throws SecurityException
    ...
    fun changeVersionTo(version: Version?, password: Password?, id: IdentificationWeb?) {
        ...
        try {
            ...
            logger.log(Log.LEVEL_INFO, "Update-Engine", "Installing Tachyon version [" + version + "] (previous version was [" + cs!!.getEngine().getInfo().getVersion() + "])")
    ...
    private fun downloadCore(factory: CFMLEngineFactory?, version: Version?, id: Identification?): File? {
        ...
        // local resource
        val patchDir: File = factory.getPatchDirectory()
        val newTachyon = File(patchDir, version.toString() + ".lco")
        var code: Int
        ...
        if (code != 200) {
            ...
            if (code != 200) {
                val msg = ("Tachyon Core download failed (response status:" + code + ") the core for version [" + version.toString() + "] from " + updateUrl
        ...
        if (newTachyon.createNewFile()) {
            IOUtil.copy(conn.getContent() as InputStream, FileOutputStream(newTachyon), false, true)
            conn.disconnect()

            // when it is a loader extract the core from it
            val tmp: File = CFMLEngineFactory.extractCoreIfLoader(newTachyon)
            if (tmp != null) {
                ...
                tmp.delete()
                // System. out.println("exist?" + newTachyon.exists()); // MUST remove
            }
        } else {
            conn.disconnect()
        ...
        synchronized(factory) {
            try {
                val m: Method = factory.getClass().getDeclaredMethod("_restart", arrayOfNulls<Class?>(0))
                        ?: throw ApplicationException("Cannot restart Tachyon.")
            ...
            while (zis.getNextEntry().also { entry = it } != null) {
                ...
                if (!entry.isDirectory() && (startsWith(path, type, "event-gateways") || startsWith(path, type, "eventGateways"))
                        && (StringUtil.endsWithIgnoreCase(path, "." + Constants.getCFMLComponentExtension())
                                || StringUtil.endsWithIgnoreCase(path, "." + Constants.getTachyonComponentExtension()))) {
                    val sub = subFolder(entry)
    ...
    private fun _removeWebContexts(config: Config?, realpath: String?, _store: Boolean): Boolean {
        if (config is ConfigServer) {
            ...
            // remove files from tachyon web context
            var store = false
            val webs: Array<ConfigWeb?> = cs.getConfigWebs()
    ...
    fun updatePlugin(pc: PageContext?, src: Resource?) {
        ...
        if (!PluginFilter.doAccept(src)) throw ApplicationException("Plugin [" + src.getAbsolutePath().toString() + "] is invalid, missing one of the following files [Action."
                + Constants.getCFMLComponentExtension().toString() + " or Action." + Constants.getTachyonComponentExtension().toString() + ",language.xml] in root, existing files are ["
                + tachyon.runtime.type.util.ListUtil.arrayToList(src.list(), ", ").toString() + "]")
        ...
        companion object {
            fun doAccept(res: Resource?): Boolean {
                return res.isDirectory() && (res.getRealResource("/Action." + Constants.getCFMLComponentExtension()).isFile()
                        || res.getRealResource("/Action." + Constants.getTachyonComponentExtension()).isFile()) && res.getRealResource("/language.xml").isFile()
            }
        }
    }
	 ...
	 */
        ...
        fun updateCore(config: ConfigServerImpl?, core: Resource?, reload: Boolean) {
            ...
            try {
                ...
                core.moveTo(pf)
                core = pf
                logger.log(Log.LEVEL_INFO, "Update-Engine", "Installing Tachyon [" + v + "] (previous version was [" + cs!!.getEngine().getInfo().getVersion() + "] )")
                //
                val admin = ConfigAdmin(config, null)
