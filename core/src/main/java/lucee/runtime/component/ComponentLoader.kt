...
object ComponentLoader {
    ...
    private fun _search(pc: PageContext?, loadingLocation: PageSource?, rawPath: String?, searchLocal: Boolean?, searchRoot: Boolean?, executeConstr: Boolean, returnType: Short,
        ...
        if (obj == null && (pc.getConfig() as ConfigPro).allowLuceeDialect()) { // only when the lucee dialect is enabled we have to check the opposite
    ...
    private fun _search(pc: PageContext?, loadingLocation: PageSource?, rawPath: String?, searchLocal: Boolean?, searchRoot: Boolean?, executeConstr: Boolean, returnType: Short,
        ...
        val pathWithCFC: String = path.concat("." + if (dialect == CFMLEngine.DIALECT_CFML) Constants.getCFMLComponentExtension() else Constants.getLuceeComponentExtension())
    ...
    private fun loadSub(page: CIPage?, sub: String?): CIPage? {
        ...
        val subClassName: String = lucee.transformer.bytecode.Page.createSubClass(page.getPageSource().getClassName(), sub, page.getPageSource().getDialect())
