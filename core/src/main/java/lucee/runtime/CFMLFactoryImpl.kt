...
class CFMLFactoryImpl(engine: CFMLEngineImpl?, sg: ServletConfig?) : CFMLFactory() {
    ...
    private var luceeExtensions: ArrayList<String?>? = null
    ...
    @Override
    @Deprecated
    fun getLuceePageContext(servlet: HttpServlet?, req: HttpServletRequest?, rsp: HttpServletResponse?, errorPageURL: String?, needsSession: Boolean, bufferSize: Int,
        ...
        if (!isChild) {
            ...
            if (ra != null) {
                ...
                if (count > 0) {
                    ...
                    val maxNormPrio: Int = Caster.toIntValue(SystemUtil.getSystemPropOrEnvVar("lucee.request.limit.concurrent.maxnormprio", null), MAX_NORMAL_PRIORITY)
                    ...
                    val maxNoSleep: Int = Caster.toIntValue(SystemUtil.getSystemPropOrEnvVar("lucee.request.limit.concurrent.maxnosleep", null), MAX_NO_SLEEP)
                    if (maxNoSleep > 0 && count >= maxNoSleep) {
                        val ms: Int = Caster.toIntValue(SystemUtil.getSystemPropOrEnvVar("lucee.request.limit.concurrent.sleeptime", null), SLEEP_TIME)
                        if (ms > 0) {
                            SystemUtil.sleep(ms)
                        }
                    }
    ...
    fun releasePageContext(pc: javax.servlet.jsp.PageContext?) {
        releaseLuceePageContext(pc as PageContext?, true)
    }
    ...
    @Override
    @Deprecated
    fun releaseLuceePageContext(pc: PageContext?) {
     ...
     */
    @Override
    fun releaseLuceePageContext(pc: PageContext?, unregisterFromThread: Boolean) {
    ...
    fun toDialect(ext: String?, defaultValue: Int): Int {
        ...
        return if (luceeExtensions.contains(ext)) CFMLEngine.DIALECT_LUCEE else defaultValue
    }

    private fun _initExtensions() {
        cfmlExtensions = ArrayList<String?>()
        luceeExtensions = ArrayList<String?>()
        try {
    ...
    @Override
    fun getLuceeExtensions(): Iterator<String?>? {
