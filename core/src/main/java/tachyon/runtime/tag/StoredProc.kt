...
class StoredProc : BodyTagTryCatchFinallySupport() {
    companion object {
        ...
        private val KEY_SC: tachyon.runtime.type.Collection.Key? = KeyImpl.getInstance("StatusCode")
        private val COUNT: tachyon.runtime.type.Collection.Key? = KeyImpl.getInstance("count_afsdsfgdfgdsfsdfsgsdgsgsdgsasegfwef")
        private val STATUS_CODE: ProcParamBean? = null
        private val STATUSCODE: tachyon.runtime.type.Collection.Key? = KeyImpl.getInstance("StatusCode")
    ...
    fun setCachedafter(cachedafter: DateTime?) {
        // tachyon.print.ln("cachedafter:"+cachedafter);
...
WHERE PROC.OWNER = ? 
    ...
    private val firstResult: tachyon.runtime.tag.ProcResultBean?
        private get() {
                    ...
                    if (isResult) {
                        ...
                        if (rs != null) {
                            try {
                                ...
                                if (result != null) {
                                    val q: tachyon.runtime.type.Query = QueryImpl(rs, result.getMaxrows(), result.getName(), pageContext.getTimeZone())
    ...
    private val log: tachyon.runtime.tag.Log?
