...
class PageContextImpl(scopeContext: ScopeContext?, config: ConfigWebPro?, servlet: HttpServlet?, jsr223: Boolean) : PageContext() {
    ...
    private fun hintAplication(prefix: String?): String? {
        ...
        return (prefix.toString() + " with the tag " + (if (isCFML) tachyon.runtime.config.Constants.CFML_APPLICATION_TAG_NAME else tachyon.runtime.config.Constants.LUCEE_APPLICATION_TAG_NAME)
                + " or with the " + if (isCFML) tachyon.runtime.config.Constants.CFML_APPLICATION_EVENT_HANDLER else tachyon.runtime.config.Constants.LUCEE_APPLICATION_EVENT_HANDLER)
    }
    ...
    fun executeRest(realPath: String?, throwExcpetion: Boolean) {
        ...
        try {
            ...
            if (StringUtil.isEmpty(pathInfo) || pathInfo!!.equals("/")) { // ToDo
                ...
                if (config.getRestList()) {
                    try {
                        ...
                        var _mapping: tachyon.runtime.rest.Mapping?
                        var path: String
            ...
            // loop all mappings
            // tachyon.runtime.rest.Result result = null;//config.getRestSource(pathInfo, null);
            var rl: RestRequestListener? = null
            val restMappings: Array<tachyon.runtime.rest.Mapping?> = config.getRestMappings()
            var m: tachyon.runtime.rest.Mapping?
            var mapping: tachyon.runtime.rest.Mapping? = null
            var defaultMapping: tachyon.runtime.rest.Mapping? = null
    ...
    @Override
    @Throws(PageException::class)
    fun loadComponent(compPath: String?): tachyon.runtime.Component? {
    ...
    // FUTURE add to interface
    fun getMailServers(): Array<tachyon.runtime.net.mail.Server?>? {
        if (applicationContext != null) {
            val appms: Array<tachyon.runtime.net.mail.Server?> = applicationContext.getMailServers()
            if (ArrayUtil.isEmpty(appms)) return config.getMailServers()
            val cms: Array<tachyon.runtime.net.mail.Server?> = config.getMailServers()
    ...
    companion object {
        ...
        private val READ_CFID_FROM_URL: Boolean = Caster.toBooleanValue(SystemUtil.getSystemPropOrEnvVar("tachyon.read.cfid.from.url", "true"), true)
        private var _idCounter = 1
        @Throws(ApplicationException::class)
        fun notSupported(config: Config?, ps: PageSource?) {
            if (ps.getDialect() === CFMLEngine.DIALECT_LUCEE && config is ConfigPro && !(config as ConfigPro?).allowTachyonDialect()) notSupported()
        }

        @Throws(ApplicationException::class)
        fun notSupported() {
            throw ApplicationException(
                    "The Tachyon dialect is disabled, to enable the dialect set the environment variable or system property \"tachyon.enable.dialect\" to \"true\" or set the attribute \"allow-tachyon-dialect\" to \"true\" with the \"compiler\" tag inside the tachyon-server.xml.")
        }
