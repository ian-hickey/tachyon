...
class LSNumberFormat : BIF() {
    ...
    companion object {
        ...
        private fun _call(pc: PageContext?, `object`: Object?, mask: String?, locale: Locale?): String? {
            return try {
                val formatter: tachyon.runtime.util.NumberFormat = NumberFormat()
                if (mask == null) return formatter!!.format(locale, tachyon.runtime.functions.displayFormatting.NumberFormat.toNumber(pc, `object`, 0))
                val m: Mask = tachyon.runtime.util.NumberFormat.convertMask(mask)
                val number: Double = tachyon.runtime.functions.displayFormatting.NumberFormat.toNumber(pc, `object`, m.right)
